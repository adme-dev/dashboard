...
import { coordinateCheckpointStaging, coordinateStaging, type StagingCoordinatorDependencies } from './stagingCoordinator'
...
export async function handleStagingManagement(input: unknown, env: Record<string, unknown>, transaction: DomainTransaction) {
  ...
  try {
    const value = await coordinateStaging(parsed.data, stagingDependencies(env, transaction))
...
function stagingDependencies(env: Record<string, unknown>, transaction: DomainTransaction): StagingCoordinatorDependencies {
  const service = env.PAGE_STUDIO_CLIENT_STAGING as { buildSnapshot: StagingCoordinatorDependencies['build'], verifySnapshot: StagingCoordinatorDependencies['verify'] }
  return {
    transaction,
...
/** Service binding only: the immutable audit selects the original login actor. */
export async function handleCheckpointStaging(input: unknown, env: Record<string, unknown>, transaction: DomainTransaction) {
  const rejected = (code: string, statusCode: number) => ({ ok: false as const, error: { code, statusCode } })
  const parsed = CheckpointStagingRequestSchema.safeParse(input)
  if (!parsed.success) return rejected('STAGING_INVALID', 400)
  const environment = env.PAGE_STUDIO_RELEASE_ENVIRONMENT
  const database = env.HYPERDRIVE_FRESH as { connectionString?: unknown } | undefined
  const service = env.PAGE_STUDIO_CLIENT_STAGING as { buildSnapshot?: unknown, verifySnapshot?: unknown } | undefined
  if (environment !== parsed.data.expectedEnvironment || typeof database?.connectionString !== 'string' || !database.connectionString
    || typeof service?.buildSnapshot !== 'function' || typeof service.verifySnapshot !== 'function') return rejected('STAGING_SERVICE_UNAVAILABLE', 503)
  try {
    const value = PageStudioStagingStateSchema.parse(await coordinateCheckpointStaging(parsed.data, environment, stagingDependencies(env, transaction)))
    if (value.siteId !== parsed.data.scope.siteId.toLowerCase()) throw new Error('Checkpoint staging response scope mismatch')
    return { ok: true as const, request: parsed.data, value }
  } catch (error) {
    if (error instanceof StagingStoreError) return rejected(error.code, error.statusCode)
    console.error(JSON.stringify({ event: 'page_studio_checkpoint_staging_failure', environment }))
    return rejected('STAGING_SERVICE_UNAVAILABLE', 503)
  }
}
