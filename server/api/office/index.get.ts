...
import type { OfficeMemberRole, OfficeRow } from '~~/app/types/office'

export default defineEventHandler(async (event) => {
  const user = await requireAuth(event)
  let offices = await queryRows<OfficeRow & { my_role: OfficeMemberRole }>(
  ...
  if (offices.length === 0 && import.meta.dev) {
    ...
    if (office) {
      const role: OfficeMemberRole = ['owner', 'admin'].includes(user.role) ? 'admin' : 'member'
      ...
      offices = await queryRows<OfficeRow & { my_role: OfficeMemberRole }>(
        `SELECT o.*, om.role AS my_role
         FROM offices o
         JOIN office_members om ON om.office_id = o.id
         WHERE om.user_id = $1
