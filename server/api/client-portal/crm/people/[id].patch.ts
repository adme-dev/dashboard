...
const AUDIT_COLS = ['company_id', 'first_name', 'last_name', 'email', 'phone', 'mobile', 'job_title', 'department', 'city', 'notes', 'lifecycle_stage', 'tags'] as const
...
export default defineEventHandler(async (event) => {
  ...
  const before = await queryOne<Record<string, unknown>>(
    `SELECT * FROM crm_people WHERE id = $1 AND client_id = $2 AND deleted_at IS NULL`, [id, client.clientId])
  const sets: string[] = []
  ...
  try {
    await recordFieldChanges({ clientId: client.clientId, entityType: 'person', entityId: id as string, before, after: row, fields: [...AUDIT_COLS], actor: client.id })
