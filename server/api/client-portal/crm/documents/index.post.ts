// server/api/client-portal/crm/documents/index.post.ts — session-scoped multipart upload.
...
export default defineEventHandler(async (event) => {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(targetId)) throw createError({ statusCode: 400, statusMessage: 'Invalid target_id' })
  ...
  await uploadFile(file.data, key, fileType, { clientId: client.clientId, targetType, targetId, originalName: fileName })

  const row = await createDocument({
    clientId: client.clientId, targetType, targetId, fileKey: key, fileName, contentType: fileType,
    sizeBytes: fileSize, documentType, expiresAt, uploadedBy: client.id,
  })
  return { item: row }
})
