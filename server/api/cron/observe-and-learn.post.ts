...
import {
  ...
  type ObserveAndLearnRuntimePolicy
} from '~~/server/utils/ai/observe/runtimePolicy'

export interface ObserveAndLearnHandlerDependencies {
  resolveRuntimePolicy(event: H3Event): ObserveAndLearnRuntimePolicy
  runObservePass: typeof runObservePass
}

const defaultDependencies: ObserveAndLearnHandlerDependencies = {
  resolveRuntimePolicy: resolveObserveAndLearnRuntimePolicy,
  runObservePass
}

export function createObserveAndLearnHandler(
  overrides: Partial<ObserveAndLearnHandlerDependencies> = {}
) {
  const dependencies: ObserveAndLearnHandlerDependencies = { ...defaultDependencies, ...overrides }
  return async (event: H3Event) => {
    ...
    // Dormancy gate — request bindings are authoritative in Cloudflare and malformed values fail shut.
    if (!dependencies.resolveRuntimePolicy(event).enabled) {
    ...
    const result = await dependencies.runObservePass(deps)
    return { ok: true, ...result }
  }
}

export default defineEventHandler(createObserveAndLearnHandler())
