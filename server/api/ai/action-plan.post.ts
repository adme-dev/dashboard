    ...
    try {
      const raw = await generateModelRoutedGroqInsight(prompt, {
        ...
        metadata: {
          ...
          hasMetric: Boolean(body.metric),
          hasRecommendation: Boolean(body.recommendation),
          actionStepCount: body.actionSteps?.length || 0,
          tagCount: body.tags?.length || 0,
          xeroDataSourceCount: xeroDataUsed.length,
