      ...
      const result = await db.query<{
        ...
        SELECT site.tenant_id, site.client_id, site.id, site.name, site.starter_version,
               ...
               (site.status IN ('draft', 'active') AND client.is_active = TRUE
                AND entitlement.status IN ('trial', 'active') AND entitlement.portal_creation_enabled
        ...
        JOIN page_studio_site_memberships membership
          ...
          AND membership.site_id = site.id AND membership.user_id = $3 AND membership.role = 'editor'
