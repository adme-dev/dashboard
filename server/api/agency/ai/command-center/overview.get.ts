...
export default defineEventHandler(async (event) => {
  ...
  if (!roleHasPermission(user.role, 'MANAGEMENT')) {
    throw createError({ statusCode: 403, statusMessage: 'Command Center requires a management role' })
  }

  // Open proposals awaiting confirmation (org-wide), newest first.
  const proposals = await queryRows<ProposalRow>(
  ...
  const audit = await queryRows<AuditRow>(
  ...
  const usageRow = await queryOne<{ turns: string, cost: string, tokens: string }>(
  ...
  const memoryRow = await queryOne<{ total: string, users: string }>(
