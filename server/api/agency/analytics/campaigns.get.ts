  ...
  try {
    ...
    const rows = await queryRows(`
      ...
      campaigns AS (
        SELECT
          ...
          (array_agg(ms.serving_status ORDER BY ms.serving_status_synced_at DESC NULLS LAST))[1] as serving_status,
          (array_agg(to_jsonb(ms.serving_status_reasons) ORDER BY ms.serving_status_synced_at DESC NULLS LAST))[1] as serving_status_reasons,
          (array_agg(to_jsonb(ms.provider_serving_status_reasons) ORDER BY ms.serving_status_synced_at DESC NULLS LAST))[1] as provider_serving_status_reasons,
          MAX(ms.serving_status_synced_at) as serving_status_synced_at,
          (array_agg(ms.serving_status_unavailable_reason ORDER BY ms.synced_at DESC NULLS LAST))[1] as serving_status_unavailable_reason,
          ...
          (array_agg(ms.impression_share ORDER BY ms.impression_share_synced_at DESC NULLS LAST))[1] as impression_share,
          (array_agg(ms.lost_impression_share_budget ORDER BY ms.impression_share_synced_at DESC NULLS LAST))[1] as lost_impression_share_budget,
          (array_agg(ms.lost_impression_share_rank ORDER BY ms.impression_share_synced_at DESC NULLS LAST))[1] as lost_impression_share_rank,
          MAX(ms.impression_share_synced_at) as impression_share_synced_at,
          (array_agg(ms.impression_share_unavailable_reason ORDER BY ms.synced_at DESC NULLS LAST))[1] as impression_share_unavailable_reason,
    ...
    const campaigns = rows.map((r: any) => {
      ...
      return {
        ...
        frequency: r.frequency == null ? null : Number(r.frequency),
        servingStatus: r.serving_status || null,
        servingStatusReasons: Array.isArray(r.serving_status_reasons) ? r.serving_status_reasons : [],
        providerServingStatusReasons: Array.isArray(r.provider_serving_status_reasons) ? r.provider_serving_status_reasons : [],
        servingStatusAsOf: r.serving_status_synced_at || null,
        servingStatusUnavailableReason: r.serving_status_unavailable_reason || null,
        impressionShare: r.impression_share == null ? null : Number(r.impression_share) / 100,
        lostImpressionShareBudget: r.lost_impression_share_budget == null ? null : Number(r.lost_impression_share_budget) / 100,
        lostImpressionShareRank: r.lost_impression_share_rank == null ? null : Number(r.lost_impression_share_rank) / 100,
        impressionShareAsOf: r.impression_share_synced_at || null,
        impressionShareUnavailableReason: r.impression_share_unavailable_reason || null,
