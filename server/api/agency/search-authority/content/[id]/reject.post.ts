...
export default eventHandler(async (event) => {
  ...
  const result = await executeSearchAuthorityMutation(event, 'version-reject', async (db) => {
    await rejectContentVersion(db, { ...parsed.data, assetId: assetId.data, actorId: user.id })
    return { id: parsed.data.versionId, ok: true, status: 'rejected' as const }
  }, async (_db, id) => ({ id, ok: true, status: 'rejected' as const }))
