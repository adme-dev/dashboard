...
export default eventHandler(async (event) => {
  try {
    ...
    const request = {
      ...
      initiatingLoginSessionHash: await bindPageStudioProvisioningLogin(event, 'agency', user.id),
    ...
    return { provisioning: { requestKey: request.requestKey, scope: request.scope, job: { ...job, actor: { kind: job.actor.kind, userId: job.actor.userId } } } }
  } catch (error) {
