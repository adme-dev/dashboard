...
import { MONDAY_OAUTH_SCOPES } from '~~/server/utils/mondayOAuth'
...
export default defineEventHandler(async (event) => {
  ...
  return {
    ...
    connection: connection ? {
      ...
      requestedPermissions: [...MONDAY_OAUTH_SCOPES],
    } : null,
    scope: scope ? { id: scope.id, boardCount: Number(scope.boardCount) } : null,
  }
})
