...
/**
 * GET /api/agency/social/linkedin/account-spend?month=X&year=Y
 * Aggregates media_spend by connection for LinkedIn ad accounts
 */
export default eventHandler(async (event) => {
  ...
  const cacheKey = `spend:linkedin:accounts:${period}`
  ...
  return cachedFetch(event, cacheKey, 300, async () => {
    const rows = await queryRows<{
      ...
      `SELECT sc.id, sc.account_id, sc.account_name, sc.status, sc.metadata,
       ...
       WHERE sc.platform = 'linkedin' AND sc.status = 'active'
