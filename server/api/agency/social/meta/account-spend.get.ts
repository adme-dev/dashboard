...
/**
 * GET /api/agency/social/meta/account-spend?month=X&year=Y
 * Aggregates media_spend by connection for Meta ad accounts
 */
export default eventHandler(async (event) => {
  ...
  const cacheKey = `spend:meta:accounts:${period}`
  ...
  const rows = await queryRows<{
    ...
    `SELECT sc.id, sc.account_id, sc.account_name, sc.status, sc.metadata,
     ...
     WHERE sc.platform = 'meta' AND (sc.status = 'active' OR ms.id IS NOT NULL)
  ...
  const unlinkedRows = await queryRows<{
    ...
    `SELECT
       CASE
         WHEN ms.client_id IS NOT NULL THEN 'unlinked:meta:client:' || ms.client_id::text
         ELSE 'unlinked:meta:unmapped'
       END as id,
       CASE
         WHEN ms.client_id IS NOT NULL THEN 'unlinked:meta:client:' || ms.client_id::text
         ELSE 'unlinked:meta:unmapped'
       END as account_id,
       CASE
         WHEN ac.name IS NOT NULL THEN ac.name || ' (unlinked Meta spend)'
         ELSE 'Unlinked Meta spend'
       END as account_name,
       'unlinked' as status,
     ...
     WHERE ms.period = $1 AND ms.platform = 'meta' AND ms.connection_id IS NULL
