...
/**
 * GET /api/agency/social/meta/insights?accountId=X&month=Y&year=Z
 * Live call to Meta API for campaign-level metrics (not cached)
 */
export default eventHandler(async (event) => {
  ...
  const campaigns = await getCampaignInsights(actId, conn.access_token, month, year)
  ...
  return {
    ...
    campaigns: campaigns.map(c => ({
      ...
      conversions: extractConversions(c.actions),
