...
/**
 * GET /api/agency/social/snapchat/account-spend?month=X&year=Y
 * Aggregates media_spend by connection for Snapchat ad accounts
 */
export default eventHandler(async (event) => {
  ...
  const cacheKey = `spend:snapchat:accounts:${period}`
  ...
  return cachedFetch(event, cacheKey, 300, async () => {
    const rows = await queryRows<{
      ...
      `SELECT sc.id, sc.account_id, sc.account_name, sc.status, sc.metadata,
       ...
       WHERE sc.platform = 'snapchat' AND sc.status = 'active'
       GROUP BY sc.id
