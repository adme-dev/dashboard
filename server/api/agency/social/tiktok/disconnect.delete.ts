...
/**
 * DELETE /api/agency/social/tiktok/disconnect?connectionId=X
 * Removes a TikTok connection and its client mappings (CASCADE)
 */
export default eventHandler(async (event) => {
  ...
  // Verify it exists and is a TikTok connection
  const conn = await queryOne<{ id: string; account_name: string }>(
    `SELECT id, account_name FROM social_connections WHERE id = $1 AND platform = 'tiktok'`,
    [connectionId]
  )
