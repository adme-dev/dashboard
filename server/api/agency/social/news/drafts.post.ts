...
export default defineEventHandler(async (event) => {
  ...
  for (const id of newsIds) {
    ...
    const post = await queryOne<{ id: string }>(
      `INSERT INTO social_posts (client_id, created_by, content, link_url, platforms, account_ids, platform_overrides, status, metadata, publish_targets)
       VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb,'draft',$8::jsonb,$9::jsonb) RETURNING id`,
      [body.clientId, user.id, content, item.source_url, platforms, accountIds.length ? accountIds : null, JSON.stringify(overrides), JSON.stringify({ source: 'mcp_news', newsItemId: item.id }), explicit ? JSON.stringify(explicit.targets) : null])
    if (post) {
