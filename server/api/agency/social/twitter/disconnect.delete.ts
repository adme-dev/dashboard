...
/**
 * DELETE /api/agency/social/twitter/disconnect?connectionId=X
 * Removes a Twitter connection and its client mappings (CASCADE)
 */
export default eventHandler(async (event) => {
  ...
  // Verify it exists and is a Twitter connection
  const conn = await queryOne<{ id: string; account_name: string }>(
    `SELECT id, account_name FROM social_connections WHERE id = $1 AND platform = 'twitter'`,
    [connectionId]
  )
