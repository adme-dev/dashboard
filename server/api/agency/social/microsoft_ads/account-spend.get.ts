...
/**
 * GET /api/agency/social/microsoft_ads/account-spend?month=X&year=Y
 * Aggregates media_spend by connection for Microsoft Ads accounts.
 */
export default eventHandler(async (event) => {
  ...
  const cacheKey = `spend:microsoft_ads:accounts:${period}`
  ...
  return cachedFetch(event, cacheKey, 300, async () => {
    const rows = await queryRows<{
      ...
      `SELECT sc.id, sc.account_id, sc.account_name, sc.status, sc.metadata,
       ...
       WHERE sc.platform = 'microsoft_ads' AND sc.status = 'active'
