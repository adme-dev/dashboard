...
/**
 * DELETE /api/agency/social/microsoft_ads/disconnect?connectionId=X
 * Removes a Microsoft Ads connection and its client mappings (CASCADE).
 */
export default eventHandler(async (event) => {
  ...
  // Verify it exists and is a Microsoft Ads connection
  const conn = await queryOne<{ id: string; account_name: string }>(
    `SELECT id, account_name FROM social_connections WHERE id = $1 AND platform = 'microsoft_ads'`,
    [connectionId]
  )
