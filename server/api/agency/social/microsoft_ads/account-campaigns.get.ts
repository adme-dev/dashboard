...
/**
 * GET /api/agency/social/microsoft_ads/account-campaigns?connectionId=X&month=Y&year=Z
 * Returns campaign rows from media_spend for a single Microsoft Ads account.
 */
export default eventHandler(async (event) => {
  ...
  const rows = await queryRows<{
    ...
    `SELECT id, campaign_id, campaign_name, actual_spend, budget_allocated, COALESCE(budget_rolling, false) as budget_rolling,
     ...
     FROM media_spend
     WHERE connection_id = $1 AND period = $2 AND platform = 'microsoft_ads'
