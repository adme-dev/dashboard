...
export default defineEventHandler(async (event) => {
  ...
  try {
    return await executeGooglePmaxPausedCreate({ event, launchId, tenantId, actorId: user.id })
  } catch (error: unknown) {
    ...
    console.error('[google-pmax-execute] provider unavailable', {
      errorClass: error instanceof Error ? error.name : 'UnknownError'
    })
    throw createError({ statusCode: 502, statusMessage: 'Google PMax paused creation is temporarily unavailable' })
  }
})
