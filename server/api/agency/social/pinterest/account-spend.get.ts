...
/**
 * GET /api/agency/social/pinterest/account-spend?month=X&year=Y
 * Aggregates media_spend by connection for Pinterest ad accounts
 */
export default eventHandler(async (event) => {
  ...
  const cacheKey = `spend:pinterest:accounts:${period}`
  ...
  return cachedFetch(event, cacheKey, 300, async () => {
    const rows = await queryRows<{
      ...
      `SELECT sc.id, sc.account_id, sc.account_name, sc.status, sc.metadata,
       ...
       WHERE sc.platform = 'pinterest' AND sc.status = 'active'
