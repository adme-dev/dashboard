...
/**
 * GET /api/agency/social/pinterest/account-campaigns?connectionId=X&month=Y&year=Z
 * Returns campaign rows from media_spend for a single Pinterest ad account
 */
export default eventHandler(async (event) => {
  ...
  const rows = await queryRows<{
    ...
    `SELECT id, campaign_id, campaign_name, actual_spend, budget_allocated, impressions, clicks,
     ...
     FROM media_spend
     WHERE connection_id = $1 AND period = $2 AND platform = 'pinterest'
