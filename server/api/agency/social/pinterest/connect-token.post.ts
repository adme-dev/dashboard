...
import { getPinterestAdAccounts } from '~~/server/utils/pinterestClient'

/**
 * POST /api/agency/social/pinterest/connect-token
 * Manual token entry — validates a Pinterest access token,
...
export default eventHandler(async (event) => {
  ...
  try {
    adAccounts = await getPinterestAdAccounts(token)
  } catch (err: any) {
  ...
  for (const acct of adAccounts) {
    await queryOne(
      ...
      [
        ...
        token,
        refreshToken,
        null, // No expiry info for manual tokens
        ['ads:read'],
        'active',
        JSON.stringify({
