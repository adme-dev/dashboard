 ...
 *
 * Kicks off Pinterest campaign spend sync in the background via waitUntil and
...
export default eventHandler(async (event) => {
  ...
  return runSpendSyncInBackground(event, {
    label: `pinterest sync-spend ${period}`,
    sync: () => syncPinterestSpend(month, year),
    kvKeys: [
      `spend:summary:${period}:all`,
      `spend:summary:${period}:pinterest`,
      `spend:pinterest:accounts:${period}`,
      `spend:daily:pinterest:${period}`,
    ],
  })
})
