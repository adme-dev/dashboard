...
export default defineEventHandler(async (event) => {
  ...
  const [totals, daily, countries, devices, os, browsers, cities, postcodes] = await Promise.all([
  ...
  return {
    ...
    range: { from, to }, daily: fillDays(from, to, daily), countries, devices, os, browsers, cities, postcodes
  }
})
