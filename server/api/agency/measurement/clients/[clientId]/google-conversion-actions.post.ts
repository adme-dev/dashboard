...
import {
  ...
  GoogleConversionActionNameSchema,
...
export default defineEventHandler(async (event) => {
  ...
  if (!parsedBody.success) {
    throw createError({ statusCode: 400, statusMessage: 'Invalid conversion-action request' })
  }
  ...
  const connection = await queryOne<ConnectionRow>(
    `SELECT sc.id, sc.client_id, sc.account_id, sc.account_name, sc.status,
      ...
      LIMIT 1`,
    [clientId, parsedBody.data.connectionId]
  )
  if (!connection) {
  ...
  try {
    ...
    if (error instanceof GoogleConversionActionDiscoveryError) {
      ...
      if (error.code === 'GOOGLE_CONVERSION_ACTION_CONFLICT') {
        throw createError({ statusCode: 409, statusMessage: 'Google conversion-action name is already incompatible' })
      }
    }
    throw createError({ statusCode: 502, statusMessage: 'Google Ads conversion action could not be provisioned' })
  }
})
