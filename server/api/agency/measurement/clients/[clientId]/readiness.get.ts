...
import { createMeasurementReadRuntime } from '~~/server/utils/measurement/runtime'
...
export default defineEventHandler(async (event) => {
  ...
  const service = createMeasurementReadRuntime()

  try {
    return await service.getReadiness(clientId)
  } catch (error) {
