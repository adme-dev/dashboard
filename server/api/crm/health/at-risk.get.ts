...
export default defineEventHandler(async (event) => {
  ...
  const ownerSql = context.actorType === 'staff' && context.visibility.ownerScoped
    ? ` AND ((s.target_type = 'person' AND (p.owner_id = $2 OR p.assigned_to = $2))
             OR (s.target_type = 'company' AND (c.owner_id = $2 OR c.assigned_to = $2)))`
    : ''
  const items = await queryRows(
    `SELECT s.target_type, s.target_id, s.total_score, s.grade,
      ...
      WHERE s.client_id = $1 AND s.score_type = 'health' AND s.grade IN ('Warm', 'Cold')
        AND (p.id IS NOT NULL OR c.id IS NOT NULL)${ownerSql}
      ORDER BY s.total_score ASC
      LIMIT 100`,
    ownerSql ? [context.clientId, context.actorId] : [context.clientId],
  )
  return { items }
})
