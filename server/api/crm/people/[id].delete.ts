...
export default defineEventHandler(async (event) => {
  ...
  await transaction(async (db) => {
    await requireCrmRecordAccess(context, { type: 'person', id: id as string }, db)
    const result = await db.query(
      `UPDATE crm_people SET deleted_at = NOW() WHERE id = $1 AND client_id = $2 AND deleted_at IS NULL RETURNING id`,
      [id, context.clientId]
    )
