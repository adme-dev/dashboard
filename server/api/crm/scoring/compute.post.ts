...
export default defineEventHandler(async (event) => {
  ...
  if (b.target_id) {
    const r = await recomputeScore({ clientId: b.client_id, targetType: b.target_type, targetId: b.target_id, reason: 'manual' })
    return { item: r }
  }
  ...
  const ids = await queryRows<{ id: string }>(
    `SELECT id FROM ${table} WHERE client_id = $1 AND deleted_at IS NULL LIMIT 1000`,
    [b.client_id],
  )
  let scored = 0
  for (const { id } of ids) {
    const r = await recomputeScore({ clientId: b.client_id, targetType: b.target_type, targetId: id, reason: 'bulk' })
    if (r) scored++
  }
