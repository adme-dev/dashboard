  ...
  const row = await queryOne<CountsRow>(
    `SELECT
       (SELECT COUNT(*) FROM crm_opportunities
          WHERE client_id = $1 AND status = 'open' AND deleted_at IS NULL)::int AS active_opps,
       (SELECT COUNT(DISTINCT o.id) FROM crm_opportunities o
          ...
          WHERE o.client_id = $1 AND o.status = 'open' AND o.deleted_at IS NULL)::int AS active_opps_with_task,
       (SELECT COUNT(*) FROM crm_people
          WHERE client_id = $1 AND deleted_at IS NULL)::int AS people,
       (SELECT COUNT(DISTINCT s.target_id) FROM crm_scores s
          JOIN crm_people p ON p.id = s.target_id AND p.deleted_at IS NULL
          WHERE s.client_id = $1 AND s.target_type = 'person')::int AS people_with_score,
       (SELECT COUNT(*) FROM crm_views WHERE client_id = $1)::int AS views,
       (SELECT COUNT(DISTINCT created_by) FROM crm_views WHERE client_id = $1)::int AS view_users,
       (SELECT COUNT(*) FROM crm_companies
          WHERE client_id = $1 AND deleted_at IS NULL)::int AS companies,
       (SELECT COUNT(*) FROM crm_merge_log WHERE client_id = $1)::int AS merges`,
    [client_id],
  )
