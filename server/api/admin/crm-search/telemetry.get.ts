...
import { loadCrmSearchTelemetry } from '~~/server/utils/crm/search/operations/health'

export function createCrmSearchTelemetryHandler(overrides: Partial<{
  requireFreshAdmin(event: H3Event): ReturnType<typeof requireFreshCrmSearchAdmin>
  setResponseHeader(event: H3Event, name: string, value: string): void
  loadTelemetry(organisationScopeId: string): Promise<unknown>
}> = {}) {
  const dependencies = {
    requireFreshAdmin: (event: H3Event) => requireFreshCrmSearchAdmin(event),
    setResponseHeader,
    loadTelemetry: loadCrmSearchTelemetry,
    ...overrides
  }
  return async (event: H3Event) => {
    ...
    return await dependencies.loadTelemetry(authority.orgId)
  }
}

export default eventHandler(createCrmSearchTelemetryHandler())
