...
export async function updatePageStudioAiUsage(
  ...
  parseUsageInput(input, session, environment)
  const run = dependencies.runTransaction ?? (work => transactionWithoutRetry(db => work(db as unknown as PageStudioControlQueryClient)))
  return run(db => updatePageStudioAiUsageInTransaction(db, input, session, environment))
}

/** Trusted SQL-only composition. Caller owns a non-retrying native transaction.
 * Acquires site then usage advisory before session authority, matching the wrapper. */
export async function updatePageStudioAiUsageInTransaction(
  db: PageStudioControlQueryClient, input: unknown, session: PageStudioSessionClaims, environment: unknown
): Promise<PageStudioAiUsageReceipt> {
  const { claims, scope, request } = parseUsageInput(input, session, environment)
