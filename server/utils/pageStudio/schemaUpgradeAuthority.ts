...
export function createSchemaUpgradeAuthority(contract: SchemaUpgradeContract) {
  const denied = () => createError({ statusCode: 403, statusMessage: `${contract.label} upgrade access denied` })
  const unavailable = () => createError({ statusCode: 503, statusMessage: `${contract.label} upgrade authority unavailable` })
  ...
  function query(request: NativeRequest, retained: boolean, lock: boolean) {
    ...
    ${retained
      ...
      AND audit.site_id=site.id AND audit.resource_id=$7 AND audit.action='content.${contract.kind}-upgrade.requested'
      AND audit.resource_type='${contract.kind}_upgrade'`
      : ''}
    WHERE site.tenant_id=$1 AND site.client_id=$2 AND site.id=$3 AND site.status IN ('draft','active')
      ...
      ${retained
        ...
        WHERE cancelled.tenant_id=site.tenant_id AND cancelled.client_id=site.client_id AND cancelled.site_id=site.id
          AND cancelled.resource_id=$7 AND cancelled.resource_type='${contract.kind}_upgrade'
          AND cancelled.action='content.${contract.kind}-upgrade.disabled')`
        : ''}
    ${lock ? 'FOR SHARE' : ''}`
  }
 ...
 * cross-store commit fence; the coordinator separately checks its current lease. */
  async function authorize(input: unknown, environment: 'staging' | 'production', dependencies: { read?: Read } = {}) {
    ...
    checkPolicy(rows)
    const saved = rows[0]!, retained = contract.schema.safeParse(saved.metadata?.intent)
