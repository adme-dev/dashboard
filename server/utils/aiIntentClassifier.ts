...
const INTENT_PATTERNS: PatternRule[] = [
  ...
  { intent: 'code_query', pattern: /\b(codebase|source\s+code|repo(sitory)?|implementation|functions?\b|methods?\b|api\s+endpoint|where\s+(is|are)\s+[\w\-]+\s+(defined|implemented|located|written)|how\s+does\s+.{1,40}?\s+(work|function)\b|files?\s+(that|which|handle)|which\s+files?)\b/i, weight: 0.8 },
]
