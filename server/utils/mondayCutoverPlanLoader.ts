...
export const MondayCutoverIdentifiersSchema = z.strictObject({
...
export async function loadMondayCutoverPlan(input: {
  ...
  const rawSourceBoard = await client.getBoard(input.boardId)
  if (!rawSourceBoard) {
  ...
  if (sourceBoard.id !== input.boardId) {
  ...
  do {
    sourcePages += 1
    const page = ExternalItemsPageSchema.parse(await client.getItems(input.boardId, {
      limit: 100,
      cursor
    }))
