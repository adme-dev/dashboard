...
import type { SearchAuthorityPublicationManifest } from '~~/shared/searchAuthorityPublication'
import type { RenderedPublication } from '~~/server/utils/searchAuthority/publicationRenderer'
import { renderPublicationRobots, renderPublicationSitemap } from '~~/server/utils/searchAuthority/publicationRenderer'
...
export async function activateSearchAuthorityPublication(
  ...
  const sitemap = renderPublicationSitemap(input.rendered.canonicalUrl)
  ...
  const manifest: SearchAuthorityPublicationManifest = {
    ...
    routes: {
      ...
      '/sitemap.xml': { key: sitemapKey, contentType: 'application/xml; charset=utf-8', etag: sha256(sitemap) },
      '/robots.txt': { key: robotsKey, contentType: 'text/plain; charset=utf-8', etag: sha256(robots) }
    },
    redirects: { '/': `/guides/${input.slug}` }
  }
