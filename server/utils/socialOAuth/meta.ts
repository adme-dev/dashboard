...
export function isSocialDmEnabled(): boolean {
  return (getCachedBinding('SOCIAL_DM_ENABLED') ?? process.env.SOCIAL_DM_ENABLED) === 'true'
}

export function isSocialUserContentEnabled(): boolean {
  return (getCachedBinding('SOCIAL_USER_CONTENT_ENABLED') ?? process.env.SOCIAL_USER_CONTENT_ENABLED) === 'true'
}
