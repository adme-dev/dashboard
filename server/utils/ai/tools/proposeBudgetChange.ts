...
const defaultDeps: ProposeBudgetChangeDeps = {
  ...
  sanityCheck: change => sanityCheckBudgetChange(change, {
    complete: prompt => generateModelRoutedGroqInsight(prompt, {
      ...
      requestId: change.campaignName,
      metadata: {
        route: 'proposeBudgetChange.sanityCheck',
        toolName: 'propose_budget_change',
        campaignNameChars: change.campaignName.length,
        platform: change.platform,
        currentDailyBudget: change.currentDailyBudget,
        newDailyBudget: change.newDailyBudget,
        pctChange: change.pctChange,
        issueType: change.issueType ?? null,
        isFromZero: change.currentDailyBudget === 0,
      },
