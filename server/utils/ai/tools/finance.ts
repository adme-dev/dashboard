...
const defaultDeps: FinanceDeps = {
  cashPosition: async (ctx) => {
    const r = await $fetch<any>('/api/xero/get-out/cash-position', { headers: ctx.event.headers as any })
    return { balance: Number(r?.cashOnHand ?? 0), runwayDays: r?.daysOfCash ?? null, risk: String(r?.band ?? 'unknown') }
  },
  outstanding: async (ctx) => {
    const r = await $fetch<any>('/api/xero/invoices', { headers: ctx.event.headers as any })
