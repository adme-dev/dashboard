...
export type AssistantCatalogCoverageStatus = 'legacy' | 'governed' | 'authenticated_core' | 'god_mode'
...
export type CatalogControlReleaseState = 'draft' | 'pilot' | 'active' | 'suspended' | 'retired'
...
function mapCatalogRow(
  ...
  return {
    ...
    releaseState: row.release_state === 'draft'
      || row.release_state === 'pilot'
 ...
 * material across every release state. LEFT JOINs preserve empty releases for fail-closed handling.
 */
    ...
    const rows = await db.queryRows<ActiveCatalogDbRow>(
      `WITH registered_pack_rows AS (
         ...
         WHERE pack_release.department_id = ANY($1::uuid[])
           AND pack_release.release_state IN ('draft', 'pilot', 'active', 'suspended', 'retired')
           AND pack_version.id = ANY($2::uuid[])
       ),
       registered_capability_rows AS (
         SELECT
         ...
         WHERE capability_release.department_id = ANY($1::uuid[])
           AND capability_release.release_state IN ('draft', 'pilot', 'active', 'suspended', 'retired')
           AND capability_version.version = (
             SELECT MAX(candidate.version)
               FROM ai_capability_releases registered_release
               JOIN ai_capability_versions candidate
                 ON candidate.id = registered_release.capability_version_id
              WHERE registered_release.capability_id = capability_release.capability_id
                AND registered_release.department_id = capability_release.department_id
           )
       )
       SELECT * FROM registered_pack_rows
       UNION ALL
       SELECT * FROM registered_capability_rows
       ORDER BY department_id, source_type, release_id, capability_sort_order, tool_sort_order`,
      [departmentIds, [...latestPackVersionIds]]
    )
...
export interface GovernedCatalogComposition<T> {
  ...
  /** Application controls bypassed by server-derived owner authority, persisted by later audit. */
...
export interface EffectiveAssistantCompositionInput<T> {
  ...
  /** Server-resolved authority must be bound to the same authenticated actor. */
  authority?: GodModeAuthority
  actorUserId?: string
}
...
const GOD_MODE_CATALOG_BYPASSES: readonly GodModeBypassedControl[] = [
  'permission',
  'feature_flag',
  'release_policy',
  'evaluation_policy',
...
function godModeComposition<T extends { name: string }>(
  registeredTools: T[],
  catalogRows: ActiveCatalogRow[]
): GovernedCatalogComposition<T> {
  const latestRows = catalogRows.filter(row =>
  ...
  return {
    ...
    instructionsPreamble: [...preambles.values()].join('\n\n'),
    budget: null,
    releaseIds: [...new Set(latestRows.map(row => row.releaseId))],
    departmentIds: [...new Set(latestRows.map(row => row.departmentId))],
    packVersionIds: [...new Set(latestRows.flatMap(row => row.packVersionId ? [row.packVersionId] : []))],
    capabilityVersionIds: [...new Set(latestRows.flatMap(row => row.capabilityVersionId ? [row.capabilityVersionId] : []))],
    modelFeatureKeys: [...new Set(latestRows.flatMap(row => [
      row.packModelFeatureKey,
      row.capabilityModelFeatureKey
    ].filter((key): key is string => Boolean(key))))],
    denials: [],
    bypassedControls: [...GOD_MODE_CATALOG_BYPASSES]
  }
}
