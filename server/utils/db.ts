...
// Non-retrying transaction boundary for non-idempotent callbacks. A driver or
// COMMIT error is surfaced without replaying mutations whose commit state may
// be unknown to the caller.
export async function transactionOnce<T>(callback: (db: Pool) => Promise<T>): Promise<T> {
  return runTransactionOnce(callback)
}

// Backward-compatible retrying transaction boundary used by existing callers.
export async function transaction<T>(callback: (db: Pool) => Promise<T>): Promise<T> {
  return withRetry(() => runTransactionOnce(callback))
}
