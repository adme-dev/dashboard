...
export type MetaPermissionOAuthIntent = 'baseline' | 'catalog'
...
export function getMetaOAuthScopes(intent: MetaPermissionOAuthIntent = 'baseline'): string[] {
  return intent === 'catalog'
    ? [...META_BASELINE_OAUTH_SCOPES, ...META_CATALOG_OAUTH_SCOPES]
    : [...META_BASELINE_OAUTH_SCOPES]
}

export function normalizeMetaOAuthIntent(value: unknown): MetaPermissionOAuthIntent {
  return value === 'catalog' || value === 'catalog_management' ? 'catalog' : 'baseline'
}
