    ...
    .map((l) => {
      ...
      if (l.type === 'bg') {
        if (l.src && l.srcType === 'video') {
          ...
          return `<div class="layer" data-id="${l.id}" style="${style}background:${l.bgColor || '#000'};"><video src="${escapeHtml(absSrc(l.src, options.baseUrl))}" muted playsinline preload="auto" style="width:100%;height:100%;object-fit:${l.fit || 'cover'};object-position:${objPos};display:block;" /></div>`
        }
        if (l.src) {
          const objPos = `${l.focalX ?? 50}% ${l.focalY ?? 50}%`
          return `<div class="layer" data-id="${l.id}" style="${style}background:${l.bgColor || '#000'};"><img src="${escapeHtml(absSrc(l.src, options.baseUrl))}" style="width:100%;height:100%;object-fit:${l.fit || 'cover'};object-position:${objPos};display:block;" /></div>`
        }
      ...
      if (l.type === 'image') {
        return `<div class="layer" data-id="${l.id}" style="${style}"><img src="${escapeHtml(absSrc(l.src, options.baseUrl))}" style="width:100%;height:100%;object-fit:${l.fit || 'cover'};" /></div>`
      }
      if (l.type === 'video') {
        const objPos = `${l.focalX ?? 50}% ${l.focalY ?? 50}%`
        return `<div class="layer" data-id="${l.id}" style="${style}"><video src="${escapeHtml(absSrc(l.src, options.baseUrl))}" muted playsinline preload="auto" style="width:100%;height:100%;object-fit:${l.fit || 'cover'};object-position:${objPos};" /></div>`
      }
      if (l.type === 'audio') {
        if (!l.src) return ''
        return `<audio data-id="${l.id}" src="${escapeHtml(absSrc(l.src, options.baseUrl))}" preload="auto"${l.loopAudio ? ' loop' : ''} style="display:none;"></audio>`
      }
      return ''
    })
    .join('\n    ')
