-- 342_ai_pilot_task_evidence_terminal_immutability.sql
-- Seal every gate-bearing terminal field while preserving replay-safe state transitions.
...
BEGIN
  ...
  IF OLD.state IN ('issued', 'started') AND NEW.state = OLD.state AND NEW IS DISTINCT FROM OLD THEN
    RAISE EXCEPTION 'pilot evidence replay must be a no-op';
  END IF;
  IF OLD.state = 'terminal' AND NEW.state = 'terminal' AND NEW IS DISTINCT FROM OLD THEN
    RAISE EXCEPTION 'terminal pilot evidence is immutable';
  END IF;
  ...
  IF OLD.state = 'terminal' AND NEW.state = 'assessed' THEN
    IF (NEW.terminal_at, NEW.terminal_outcome, NEW.terminal_error_code,
