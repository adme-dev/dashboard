...
describe('AI governance pilot metrics UI', () => {
  it('renders loading, retryable error, empty, and stale states without blank content', () => {
    const retry = vi.fn()
    const loading = mount({ data: null, pending: true, error: null })
    expect(loading.host.textContent).toContain('Loading evidence')
    loading.app.unmount()

    const failed = mount({ data: null, pending: false, error: 'Evidence refresh failed', onRefresh: retry })
    expect(failed.host.textContent).toContain('Pilot evidence unavailable')
    failed.host.querySelector('button')?.click()
    expect(retry).toHaveBeenCalledOnce()
    failed.app.unmount()

    const empty = mount({ data: { generatedAt: '2026-08-03T00:00:00.000Z', window: WINDOW, metrics: [] }, pending: false, error: null })
    expect(empty.host.textContent).toContain('No mapped pilot releases')
    empty.app.unmount()

    const stale = mount({ data: { generatedAt: '2026-08-03T00:00:00.000Z', window: WINDOW, metrics: [passingMetric] }, pending: false, error: 'Refresh failed' })
  ...
  it('loads a bounded default window on the governance page and retains stale metrics after retry failure', async () => {
    ...
    ;(globalThis as any).$fetch = vi.fn(async (url: string, options?: any) => {
      ...
      if (url.endsWith('/pilot-metrics')) {
        ...
        return { generatedAt: '2026-08-03T00:00:00.000Z', window: WINDOW, metrics: [passingMetric] }
      }
