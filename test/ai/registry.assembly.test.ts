...
const WRITE_TOOLS = ['create_task', 'propose_schedule_post', 'propose_budget_alert']
...
describe('assembled tool registry (Slices 1–2 + memory + media-buyer + Phase-2 writes)', () => {
  ...
  it('the mutating tools are exactly the propose→confirm writes', () => {
    expect(registry.filter(t => t.mutates).map(t => t.name).sort()).toEqual(['create_task', 'propose_budget_alert', 'propose_schedule_post'])
  })
  ...
  it('owner sees all 19', () => {
    expect(filterToolsForUser(registry, 'owner')).toHaveLength(19)
  })
