...
describe('jobs list mapping', () => {
  it('maps rows for the project list (status + output asset surfaced)', () => {
    const job = mapVideoGenerationJobRow({
      ...
      mode: 'image-to-video', model_id: 'aigateway/seedance-i2v', provider: 'aigateway', prompt: 'x',
    ...
    expect(job).toMatchObject({ id: 'j1', status: 'succeeded', outputAssetId: 'a1', modelId: 'aigateway/seedance-i2v' })
