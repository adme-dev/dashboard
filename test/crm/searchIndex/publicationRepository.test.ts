...
describe('CRM search transport publication repository', () => {
  it('claims only bounded pending-transport work with its own SKIP LOCKED lease', async () => {
    ...
    expect(sql).toContain('control.state IN (\'enabled\', \'delete_only\')')
    expect(sql).toContain('crm_search_client_teardowns')
    expect(sql).toContain('crm_search_teardown_vectors')
    expect(sql).toContain('vector.organisation_scope_id = operation.organisation_scope_id')
    expect(sql).toContain('vector.client_id = operation.client_id')
    expect(sql).toContain('vector.entity_type = operation.entity_type')
    expect(sql).toContain('vector.entity_id = operation.entity_id')
    expect(sql).toContain('vector.schema_version = operation.schema_version')
    expect(sql).toContain('vector.vector_id = operation.vector_id')
    expect(sql).toContain('vector.namespace = operation.namespace')
    expect(sql).toContain('teardown.state IN (\'deleting\', \'provider_pending\')')
    expect(sql).toContain('teardown.provider_deletion_state IN (\'pending\', \'partially_confirmed\')')
    expect(sql).toContain('vector.deletion_state IN (\'pending\', \'provider_pending\', \'failed\')')
    expect(sql).toContain('control.state = \'enabled\'\n                  AND control.indexing_ready = TRUE')
    expect(sql).not.toMatch(/control\.state = 'delete_only'\s+OR EXISTS/)
