...
describe('client competitor nomination permission contract', () => {
  it('keeps competitor nomination explicitly opt-in across portal user persistence and responses', () => {
    ...
    const agencyLogin = source('server/api/agency/client-portal/auth/login.post.ts')
    ...
    const portalLogin = source('server/api/portal/auth/login.post.ts')
    ...
    for (const endpoint of [agencyUsers, agencyLogin, agencyMe, agencyUser, portalLogin, portalUsers]) {
    ...
    expect(agencyLogin).toContain('canNominateCompetitors: user.can_nominate_competitors ?? false')
    ...
    expect(portalLogin).toContain('canNominateCompetitors: user.can_nominate_competitors ?? false')
