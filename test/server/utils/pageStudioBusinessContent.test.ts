...
it('ignores historical static maps and only uses the scoped router', async () => {
  ...
  await readPageStudioBusinessContent({ actor, siteId, env }, { query })
...
it('freshly checks active client, entitlement association and scoped membership', async () => {
  ...
  expect(query.mock.calls[0][0]).toContain('membership.user_id = $3')
...
it.each([
  ...
  await expect(writePageStudioBusinessContent({ actor, siteId, env, body: { collections: [], expectedRevision: 0 } }, { query })).rejects.toMatchObject({ statusCode: 502, code: 'CONTENT_RESPONSE_INVALID' })
})
