...
describe('fetchGoogleRecommendations', () => {
  it('queries recommendations + optimization score and returns a normalized result', async () => {
    ...
    expect(ofetchMock.mock.calls[1][1].body.query).toContain('metrics.optimization_score_url')
    expect(ofetchMock.mock.calls[1][1].body.query).not.toContain('customer.optimization_score_url')
  })
