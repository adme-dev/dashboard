...
import { preparePageStudioCollectionStagingUpgrade } from '~~/server/utils/pageStudio/collectionStagingUpgradeIntent'
...
vi.mock('~~/server/utils/db', () => ({
  transactionWithoutRetry: () => {
    throw new Error('Inject disposable transaction')
  },
  queryRowsFresh: () => {
    throw new Error('Inject disposable query')
  }
}))
...
if (databaseUrl) {
  ...
  if (
    ...
    || !/^\/studio_cms_activation(?:_[a-z0-9_]+)?$/.test(url.pathname)
    || url.search
  )
    throw new Error('Disposable localhost collection database required')
}
const userId = '30000000-0000-4000-8000-000000000501',
  clientId = '20000000-0000-4000-8000-000000000501'
...
const migrations = [
  ...
  readFileSync(new URL(`../../../server/database/migrations/${file}`, import.meta.url), 'utf8')
)
...
describe.runIf(Boolean(databaseUrl))('native collection upgrade intent on PostgreSQL', () => {
  ...
  beforeEach(async () => {
    ...
    const entitlement = (
      await db.query(
        `INSERT INTO page_studio_entitlements(tenant_id,client_id,created_by,plan_metadata)
      VALUES('collection-fixture',$1,$2,'{"allowedModules":["business-content"],"builder":{"collectionSchemas":true}}') RETURNING id`,
        [clientId, userId]
      )
    ).rows[0]
    siteId = (
      ...
      VALUES('collection-fixture',$1,$2,'Collection fixture','collection-fixture','fixture-v1',$3) RETURNING id`,
        [clientId, entitlement.id, userId]
      )
    ).rows[0].id
    await db.query(
      'INSERT INTO page_studio_site_memberships(tenant_id,client_id,site_id,user_id,role) VALUES(\'collection-fixture\',$1,$2,$3,\'editor\')',
  ...
  const count = async () =>
    Number(
      (
        await db.query(
          'SELECT count(*) FROM page_studio_audit_events WHERE action=\'content.collection-staging-upgrade.requested\''
        )
      ).rows[0].count
    )
  ...
  const authorize = (intent: unknown) =>
  ...
  describe.each(['agency', 'client'] as const)('%s', (role) => {
    beforeEach(async () => {
      ...
      if (role === 'client')
        await db.query(
          'INSERT INTO client_sessions VALUES($1,$2,clock_timestamp()+INTERVAL \'1 day\')',
    ...
    function request() {
      ...
      incoming.url = '/test/collection-staging-upgrade'
      incoming.headers = { authorization: `Bearer ${token}` }
      const actor
        = role === 'agency'
          ? { role, actorId: userId, tenantId: 'collection-fixture', canEdit: true }
          : { role, actorId: userId, clientId }
      return {
        actor,
        event: createEvent(incoming, new ServerResponse(incoming)),
        siteId,
    ...
    async function options() {
      return {
        runTransaction: transaction(await connect()),
        resolveDatabase: vi.fn(async () => target())
      }
    }
    function deployedRequest() {
      ...
      const binding = {
        ...
        readProvisioning: vi.fn(),
        readCollectionStagingUpgradeDatabase: vi.fn(async (_scope: unknown) => target())
      }
      input.event.context.cloudflare = {
        env: { PAGE_STUDIO_PROVISIONING_ENVIRONMENT: 'staging', PAGE_STUDIO_PROVISIONER: binding }
      }
    ...
    it('discovers the exact database through the deployed binding after native admission', async () => {
      ...
      binding.readCollectionStagingUpgradeDatabase.mockImplementation(async (scope) => {
      ...
      const saved = await preparePageStudioCollectionStagingUpgrade(input, { runTransaction })
      expect(saved.intent.databaseId).toBe(target().databaseId)
      expect(await authorize(saved.intent)).toEqual(saved.intent)
      expect(await preparePageStudioCollectionStagingUpgrade(input, { runTransaction })).toEqual(
        saved
      )
      expect(binding.readCollectionStagingUpgradeDatabase).toHaveBeenCalledTimes(2)
      expect(binding.createProvisioning).not.toHaveBeenCalled()
      expect(await count()).toBe(1)
    })
    it.each(['missing-environment', 'wrong-environment', 'missing-binding', 'missing-method'])(
      'denies %s without discovery or saving intent',
      async (failure) => {
        const { input, binding } = deployedRequest()
        ...
        if (failure === 'missing-method')
          env.PAGE_STUDIO_PROVISIONER = {
            createProvisioning: binding.createProvisioning,
            readProvisioning: binding.readProvisioning
          }
        const { runTransaction } = await options()
        await expect(
          preparePageStudioCollectionStagingUpgrade(input, { runTransaction })
        ).rejects.toMatchObject({ statusCode: 503 })
        expect(binding.readCollectionStagingUpgradeDatabase).not.toHaveBeenCalled()
        expect(await count()).toBe(0)
      }
    )
    it.each(['missing', 'foreign', 'unavailable'])(
      'denies %s database discovery without saving intent',
      async (failure) => {
        const { input, binding } = deployedRequest()
        if (failure === 'missing')
          binding.readCollectionStagingUpgradeDatabase.mockResolvedValue(null as never)
        if (failure === 'foreign')
          binding.readCollectionStagingUpgradeDatabase.mockResolvedValue({
            ...target(),
            scope: { ...target().scope, siteId: randomUUID() }
          })
        if (failure === 'unavailable')
          binding.readCollectionStagingUpgradeDatabase.mockRejectedValue(
        ...
        expect(binding.readCollectionStagingUpgradeDatabase).toHaveBeenCalledOnce()
        expect(await count()).toBe(0)
      }
    )
    ...
    it('rechecks logout after deployed Worker discovery before saving intent', async () => {
      const { input, binding } = deployedRequest()
      binding.readCollectionStagingUpgradeDatabase.mockImplementation(async () => {
    ...
    it('persists one exact request, admits its original login, and preserves site state on retry', async () => {
      ...
      const saved = await preparePageStudioCollectionStagingUpgrade(input, opts)
      expect(saved.intent.scope).toEqual(target().scope)
      expect(saved.intent.actor.loginSessionHash).toBe(
        createHash('sha256').update(token).digest('hex')
      )
      expect(await authorize(saved.intent)).toEqual(saved.intent)
      expect(await preparePageStudioCollectionStagingUpgrade(input, opts)).toEqual(saved)
      expect(await count()).toBe(1)
    ...
    it('serializes concurrent retry IDs into a single native intent', async () => {
      ...
      const results = await Promise.all([
        preparePageStudioCollectionStagingUpgrade(input, a),
        preparePageStudioCollectionStagingUpgrade(input, b)
      ])
    ...
    it('denies other requests, a replacement database and a replacement login', async () => {
      const input = request(),
        opts = await options()
      await preparePageStudioCollectionStagingUpgrade(input, opts)
      await expect(
        preparePageStudioCollectionStagingUpgrade(request(), opts)
      ).rejects.toMatchObject({ statusCode: 409 })
      opts.resolveDatabase.mockResolvedValue({ ...target(), databaseId: randomUUID() })
      await expect(preparePageStudioCollectionStagingUpgrade(input, opts)).rejects.toMatchObject({
        statusCode: 409
      })
      opts.resolveDatabase.mockResolvedValue(target())
      token
        = role === 'agency'
          ? await createJwt({ userId, role: 'owner', loginInstance: randomUUID() })
          : randomUUID()
      if (role === 'client')
        await db.query(
          'INSERT INTO client_sessions VALUES($1,$2,clock_timestamp()+INTERVAL \'1 day\')',
      ...
      await expect(
        preparePageStudioCollectionStagingUpgrade({ ...request(), body: input.body }, opts)
      ).rejects.toMatchObject({ statusCode: 409 })
    })
    it.each(['scope', 'actor', 'targetDigest', 'databaseId', 'approved'])(
      'rejects browser supplied %s before database discovery',
      async (key) => {
        const input = request(),
          opts = await options()
        await expect(
          preparePageStudioCollectionStagingUpgrade(
            { ...input, body: { ...input.body, [key]: 'forged' } },
            opts
          )
    ...
    it('denies foreign scope discovery and a missing native login', async () => {
      ...
      opts.resolveDatabase.mockResolvedValue({
        ...target(),
        scope: { ...target().scope, siteId: randomUUID() }
      })
      await expect(
      ...
      await expect(
        preparePageStudioCollectionStagingUpgrade({ ...request(), event: undefined }, opts)
    ...
    it.each([
      ...
      const saved = await preparePageStudioCollectionStagingUpgrade(request(), await options())
    ...
    it('denies forged retained target, actor, scope and duplicate intents', async () => {
      const saved = await preparePageStudioCollectionStagingUpgrade(request(), await options())
      for (const input of [
        { ...saved.intent, databaseId: randomUUID() },
        { ...saved.intent, actor: { ...saved.intent.actor, loginSessionHash: 'f'.repeat(64) } },
        { ...saved.intent, scope: { ...saved.intent.scope, businessId: 'foreign' } }
      ])
        await expect(authorize(input)).rejects.toMatchObject({ statusCode: 403 })
      await db.query(
        'INSERT INTO page_studio_audit_events(tenant_id,client_id,site_id,actor_id,actor_role,action,resource_type,resource_id,metadata) SELECT tenant_id,client_id,site_id,actor_id,actor_role,action,resource_type,resource_id,metadata FROM page_studio_audit_events WHERE action=\'content.collection-staging-upgrade.requested\''
      )
      await expect(authorize(saved.intent)).rejects.toMatchObject({ statusCode: 403 })
    })
    it('retains cancellation as an appended audit event and denies retries', async () => {
      ...
      const saved = await preparePageStudioCollectionStagingUpgrade(input, opts)
      await db.query(
        'INSERT INTO page_studio_audit_events(tenant_id,client_id,site_id,actor_id,actor_role,action,resource_type,resource_id,metadata) SELECT tenant_id,client_id,site_id,actor_id,actor_role,\'content.collection-staging-upgrade.disabled\',resource_type,resource_id,metadata FROM page_studio_audit_events WHERE action=\'content.collection-staging-upgrade.requested\''
      )
      await expect(authorize(saved.intent)).rejects.toMatchObject({ statusCode: 403 })
      await expect(preparePageStudioCollectionStagingUpgrade(input, opts)).rejects.toMatchObject({
        statusCode: 403
    ...
    it('recovers after the native intent commits but its acknowledgement is lost', async () => {
      ...
      await expect(preparePageStudioCollectionStagingUpgrade(input, opts)).rejects.toThrow(
        'lost acknowledgement'
      )
      const saved = (
        await db.query(
          'SELECT metadata FROM page_studio_audit_events WHERE action=\'content.collection-staging-upgrade.requested\''
        )
      ).rows[0].metadata
      expect(await preparePageStudioCollectionStagingUpgrade(input, opts)).toEqual({
        intent: saved.intent,
    ...
    it('rolls back intent when package expires during its insert', async () => {
      await db.query(`CREATE FUNCTION delay_collection_intent() RETURNS trigger AS $$
        BEGIN IF NEW.action='content.collection-staging-upgrade.requested' THEN PERFORM pg_sleep(0.4); END IF; RETURN NEW; END; $$ LANGUAGE plpgsql;
      ...
      opts.resolveDatabase.mockImplementation(async () => {
        await db.query(
          'UPDATE page_studio_entitlements SET effective_until=clock_timestamp()+INTERVAL \'0.3 seconds\''
        )
        return target()
      })
      await expect(
