...
describe.runIf(Boolean(url))('draft history on disposable PostgreSQL', () => {
  ...
  const request = () => ({ actor, siteId: scope.siteId, bucket })
  ...
  beforeEach(async () => {
    ...
    blobs.clear()
    await db.query('TRUNCATE agency_clients, page_studio_entitlements, page_studio_sites, page_studio_site_memberships, page_studio_checkpoints, page_studio_versions, page_studio_audit_events, page_studio_release_pointers')
  ...
  it('saves a named version from the expected draft and retries after another edit', async () => {
    ...
    const result = await mutatePageStudioHistory({ ...request(), actor: portal, body }, { runTransaction })
    expect((await db.query('SELECT summary, status FROM page_studio_versions')).rows).toEqual([{ summary: body.name, status: 'draft' }])
    await db.query('UPDATE page_studio_sites SET current_checkpoint_id=\'newer\'')
    expect(await mutatePageStudioHistory({ ...request(), actor: portal, body }, { runTransaction })).toMatchObject({ versionId: result.versionId, isCurrent: false })
  ...
  it('allows viewers to read but prevents writes and cross-client reads', async () => {
    ...
    expect(await readPageStudioHistory({ ...request(), actor: portal, query: {} }, { runTransaction })).toMatchObject({ canEdit: false })
    await expect(mutatePageStudioHistory({ ...request(), actor: portal, body: restore() }, { runTransaction })).rejects.toMatchObject({ statusCode: 403 })
  ...
  it.each(['suspended', 'expired', 'archived', 'inactive', 'revoked'])('blocks mutations after %s access changes', async (reason) => {
    ...
    await expect(mutatePageStudioHistory({ ...request(), actor: portal, body: restore() }, { runTransaction })).rejects.toMatchObject({ statusCode: reason === 'inactive' ? 404 : 403 })
    expect(blobs.size).toBe(2)
  })
  ...
  it('holds the site and membership locks through the restored checkpoint commit', async () => {
    ...
    await mutatePageStudioHistory({ ...request(), actor: portal, body: restore() }, { runTransaction })
  })
})
