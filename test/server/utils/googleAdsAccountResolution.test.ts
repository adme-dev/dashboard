...
describe('Google Ads account resolution', () => {
  ...
  it('filters ambiguous matches to an explicitly authorized client scope', async () => {
    const otherClientId = 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'
    const deps = dependencies(async () => [
      ...candidates,
      {
        clientId: otherClientId,
        canonicalName: 'Northern GAC Imposter',
        aliasId: null,
        matchedName: 'Northern GAC',
        matchKind: 'canonical'
      }
    ...
    expect(result.matchedName).toBe('Northern GAC')
  })
})
