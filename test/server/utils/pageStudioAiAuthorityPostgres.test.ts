...
import {
  acceptPageStudioAiProposal, recordPageStudioCheckpoint,
  type PageStudioAiProposalAcceptanceInput, type PageStudioCheckpointInput,
...
if (databaseUrl) {
  ...
  if (!['postgres:', 'postgresql:'].includes(target.protocol)
    ...
    || !/^\/studio_ai_authority(?:_[a-z0-9_]+)?$/.test(target.pathname) || target.search) {
    throw new Error('AI authority tests require an explicitly disposable localhost studio_ai_authority database')
  }
}
...
describe.runIf(Boolean(databaseUrl))('AI acceptance authority at the PostgreSQL commit boundary', () => {
  let observer: pg.Client
  let schema: string
  let scope: PageStudioControlScope
  let claims: PageStudioSessionClaims
  let connections: pg.Client[]
  let input: PageStudioAiProposalAcceptanceInput
  ...
  async function seedSession(role: 'agency' | 'client') {
    ...
    input = { authorRole: role, baseDigest: 'a'.repeat(64), expectedCheckpointId: 'checkpoint_base',
      checkpoint: checkpoint('checkpoint_ai_proposal', 'b'), idempotencyKey: 'accept_ai_proposal', summary: 'Synthetic acceptance' }
  }

  beforeEach(async () => {
    connections = []
    schema = `ai_authority_${randomUUID().replaceAll('-', '')}`
    ...
    const entitlement = (await observer.query(`INSERT INTO page_studio_entitlements(tenant_id,client_id,created_by)
      VALUES('ai-authority',$1,$2) RETURNING id`, [clientId, userId])).rows[0]
    const site = (await observer.query(`INSERT INTO page_studio_sites(tenant_id,client_id,entitlement_id,name,route,starter_version,created_by)
      VALUES('ai-authority',$1,$2,'AI authority fixture','ai-authority','fixture-v1',$3) RETURNING id`, [clientId, entitlement.id, userId])).rows[0]
    scope = { tenantId: 'ai-authority', clientId, siteId: site.id }
  ...
  describe.each(['agency', 'client'] as const)('%s session', (role) => {
    ...
    it('rejects a waiting save after actual logout commits its revocation and foreign-key-backed audit', async () => {
      await assertPageStudioSessionAuthority(claims, 'model:invoke', { queryOneFresh })
    ...
    it('makes actual logout wait for an authorized save, then records revocation without a foreign-key deadlock', async () => {
      ...
      const options = { session: claims, runTransaction: transactionFor(writer, async () => {
        readyToCommit = true
      ...
      try {
        ...
        await expect(assertPageStudioSessionAuthority(claims, 'model:invoke', { queryOneFresh })).rejects.toMatchObject(denied)
      } finally {
        release.resolve()
        await save
        await logout
    ...
    const commonChanges = [
      ['editor revocation', 'UPDATE page_studio_sessions SET revoked_at=clock_timestamp()'],
      ['parent login revocation', 'UPDATE page_studio_login_sessions SET revoked_at=clock_timestamp()'],
      ['client account deactivation', 'UPDATE agency_clients SET is_active=FALSE'],
      ['model allowance removal', 'UPDATE page_studio_entitlements SET monthly_ai_operation_limit=0']
    ]
    const actorChanges = role === 'agency'
      ? [
          ['staff deactivation', 'UPDATE team_members SET is_active=FALSE'],
          ['staff role read-only', 'UPDATE custom_roles SET is_read_only=TRUE'],
          ['permission removal', 'DELETE FROM role_permission_groups WHERE permission_group=\'PAGE_STUDIO_EDIT\'']
        ]
      : [
          ['portal user deactivation', 'UPDATE client_users SET status=\'disabled\''],
          ['native portal logout', 'DELETE FROM client_sessions'],
          ['membership removal', 'DELETE FROM page_studio_site_memberships']
        ]
    it.each([...commonChanges, ...actorChanges])('holds %s authority until an already-authorized save commits', async (_name, mutation) => {
      const before = await snapshot()
      const writer = await connect()
      const revoker = await connect()
      const release = deferred()
      ...
      const save = capture(acceptPageStudioAiProposal(input, options))
      let revocation: Promise<Outcome<pg.QueryResult>> | undefined
      try {
        await expect.poll(() => readyToCommit, { timeout: 1500 }).toBe(true)
        expect(await snapshot()).toEqual(before)
        revocation = capture(revoker.query(mutation!))
        await waitForBlock(revoker, writer)
        release.resolve()
        expect(await save).toMatchObject({ ok: true, value: { acknowledged: true } })
    ...
    it('preserves authorized replay idempotency but rejects replay after revocation without new audit or mutation', async () => {
      ...
      expect(await snapshot()).toEqual(before)
      await observer.query('UPDATE page_studio_sessions SET revoked_at=clock_timestamp() WHERE nonce=$1', [claims.nonce])
      await expect(acceptPageStudioAiProposal(input, options)).rejects.toMatchObject(denied)
      expect(await snapshot()).toEqual(before)
    })
  })
})
