...
const migration = readFileSync(
  new URL('../../../../server/database/migrations/283_client_lead_capture_mode.sql', import.meta.url),
  'utf8'
)
const reconciliationMigration = readFileSync(
  new URL('../../../../server/database/migrations/285_lead_submission_intent_reconciliation.sql', import.meta.url),
  'utf8'
)

describe('lead capture contract', () => {
  it('defines server-owned client modes and preserves existing CRM adopters', () => {
    expect(migration).toContain("'analytics_only', 'capture_only', 'full_crm'")
