...
const row = () => ({ ...scope, name: 'Portal Limo Fixture', membershipRole: 'editor', starterVersion: 'limousine-v1', revision: null, status: null, source: null, brief: null, plan: null })
...
describe('portal website setup state', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mocks.access.mockResolvedValue({ clientId, id: '30000000-0000-4000-8000-000000000901', role: 'manager' })
  ...
  it('returns an empty proposal for a scoped draft, without reading a nonexistent job', async () => {
    ...
    expect(mocks.queryOneFresh.mock.calls[0][1]).toEqual([clientId, siteId, '30000000-0000-4000-8000-000000000901'])
  ...
  it.each(['viewer', 'member'])('does not offer editing to portal role %s', async (role) => {
    mocks.access.mockResolvedValue({ clientId, id: '30000000-0000-4000-8000-000000000901', role })
    const { default: handler } = await import('~~/server/api/portal/page-studio/sites/[siteId]/setup-proposal.get')
    await expect(handler(event() as never)).resolves.toMatchObject({ canEdit: false })
  })
