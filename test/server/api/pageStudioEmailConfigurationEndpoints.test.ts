...
vi.mock('~~/server/utils/pageStudio/emailConfiguration', async original => ({
  ...await original<typeof import('~~/server/utils/pageStudio/emailConfiguration')>(),
  readPageStudioEmailConfiguration: mocks.read, writePageStudioEmailConfiguration: mocks.write
}))

const settings = { senderName: 'Synthetic Fleet', fromAddress: 'sender@example.invalid', replyTo: 'reply@example.invalid', notificationRecipient: 'staff@example.invalid', inboundAddress: '', forwardingDestination: '' }
...
function request(audience: 'agency' | 'portal', method: 'GET' | 'PUT', body?: string, contentType = 'application/json') {
  ...
  router.add('/sites/:siteId/email', event => handlePageStudioEmailConfiguration(event, audience, method), [method.toLowerCase() as 'get' | 'put'])
  const fetch = toWebHandler(createApp().use(router))
  return fetch(new Request(`https://example.test/sites/${siteId}/email`, {
    method, body, headers: body === undefined ? {} : { 'content-type': contentType }
  }))
}

describe('email settings HTTP authentication and body boundary', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    ...
    mocks.read.mockResolvedValue({ settings: null, revision: 0 })
  ...
  it.each(['agency', 'portal'] as const)('uses authenticated %s identity for reads and writes', async (audience) => {
    ...
    expect((await request(audience, 'PUT', JSON.stringify({ settings, expectedRevision: 0 }))).status).toBe(200)
    ...
    expect(mocks.write).toHaveBeenCalledWith(expect.objectContaining({ actor: expected, siteId, body: { settings, expectedRevision: 0 } }))
    if (audience === 'agency') {
  ...
  it('keeps viewing available when editing permission is denied', async () => {
    ...
    expect((await request('agency', 'PUT', '{}')).status).toBe(403)
    expect(mocks.write).not.toHaveBeenCalled()
  })
  it('rejects oversized, malformed and non-JSON bodies before the service', async () => {
    expect((await request('portal', 'PUT', 'x'.repeat(8193))).status).toBe(413)
