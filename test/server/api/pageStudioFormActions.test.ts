...
vi.mock('~~/server/utils/pageStudio/formActionDiscovery', () => ({
  readAcceptedFormActions: mocks.read
}))
...
it('derives the existing verified child principal without model invocation authority', async () => {
  expect(await handleStudioFormActions(event as never)).toEqual({ version: 1, actions: [] })
...
it.each(['{"scope":{}}', '{"actor":"admin"}', '{"actionPin":{}}'])(
  ...
  async (raw) => {
    body(raw)
    await expect(handleStudioFormActions(event as never)).rejects.toMatchObject({ statusCode: 400 })
    expect(mocks.read).not.toHaveBeenCalled()
  }
)
it('bounds the actual request stream', async () => {
  body(' '.repeat(4097))
  await expect(handleStudioFormActions(event as never)).rejects.toMatchObject({ statusCode: 413 })
  expect(mocks.read).not.toHaveBeenCalled()
})
it('denies failed machine or child authentication before private discovery', async () => {
  ...
  await expect(handleStudioFormActions(event as never)).rejects.toThrow('Machine denied')
  mocks.verify.mockRejectedValueOnce(new Error('Child revoked'))
  await expect(handleStudioFormActions(event as never)).rejects.toThrow('Child revoked')
  expect(mocks.read).not.toHaveBeenCalled()
})
