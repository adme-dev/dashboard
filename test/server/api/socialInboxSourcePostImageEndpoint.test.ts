...
const mockFetchMetaSourcePostImageUrl = vi.fn()
...
vi.mock('~~/server/utils/socialInbox/sourcePostMedia', () => ({
  fetchMetaSourcePostImageUrl: (...args: unknown[]) => mockFetchMetaSourcePostImageUrl(...args)
}))
...
describe('GET /api/agency/social/inbox/conversations/:id/source-post-image', () => {
  it('redirects to a fresh allowlisted Meta CDN URL', async () => {
    mockFetchMetaSourcePostImageUrl.mockResolvedValue(
      'https://scontent-syd2-1.xx.fbcdn.net/fresh/post.jpg'
    )

    const response = await handler({ id: 'conversation-1', context: {} } as never) as Response

    expect(response.status).toBe(302)
    expect(response.headers.get('location')).toBe('https://scontent-syd2-1.xx.fbcdn.net/fresh/post.jpg')
    expect(response.headers.get('cache-control')).toBe('private, no-store')
    expect(mockFetchMetaSourcePostImageUrl).toHaveBeenCalledWith({
      platform: 'facebook',
  ...
  it('returns not found when the conversation has no refreshable image', async () => {
    mockFetchMetaSourcePostImageUrl.mockResolvedValue(null)
