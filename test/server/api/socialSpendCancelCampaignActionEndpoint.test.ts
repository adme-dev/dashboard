...
describe('POST /api/agency/social/spend/:id/actions/:actionId/cancel', () => {
  ...
  it('cancels a planned or approved action without deleting its history row', async () => {
    ...
    expect(String(mockQueryOne.mock.calls[0][0])).toContain('cancelled_by = $3::uuid')
    expect(String(mockQueryOne.mock.calls[0][0])).toContain("'cancelledBy', ($3::uuid)::text")
    expect(String(mockQueryOne.mock.calls[0][0])).toContain("'proposalDecidedBy', ($3::uuid)::text")
