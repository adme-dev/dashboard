...
describe('agency client portal dashboard API', () => {
  ...
  it('returns enterprise operating health for an agency-opened client portal', async () => {
    ...
    expect(leadSql).toContain('JOIN lead_rule_destinations d ON d.rule_id = r.id')
    expect(leadSql).toContain('r.source = l.source')
    expect(leadSql).toContain('r.form_id = l.form_id')
