...
vi.mock('~~/server/utils/pageStudio/contentAttachmentCompletion', () => ({ commitPageStudioContentAttachment: mocks.authorize }))
...
describe('internal CMS attachment authority endpoint', () => {
  ...
  it('authenticates before reading user input or accessing authority', async () => {
    ...
    const { default: handler } = await import('~~/server/routes/internal/page-studio/content-attachments/complete.post')
  ...
  it('uses only the trusted binding and prevents caching of the authority result', async () => {
    const binding = { readProvisioning: vi.fn(), createProvisioning: vi.fn(), readContentAttachmentPreparation: vi.fn() }
    ...
    expect(mocks.authorize).toHaveBeenCalledWith(body, 'staging', { binding })
  })
})
