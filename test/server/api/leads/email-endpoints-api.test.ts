...
const listEmailEndpointsForActor = vi.fn()
...
describe('GET /api/leads/email-endpoints', () => {
  ...
  it('returns 403 for a portal session without calling staff authorization or enumeration', async () => {
    ...
    expect(listEmailEndpointsForActor).not.toHaveBeenCalled()
  })

  it('requires the MEDIA_BUYING role before returning one actor-scoped batch', async () => {
    const handler = (await import('~~/server/api/leads/email-endpoints/index.get')).default
    await handler({ context: {}, query: {} } as never)
    expect(requireRole).toHaveBeenCalledOnce()
    expect(listEmailEndpointsForActor).toHaveBeenCalledWith('22222222-2222-4222-8222-222222222222')
    expect(listEmailEndpoints).not.toHaveBeenCalled()
  })

  it('retains the explicitly authorized per-client query for focused callers', async () => {
    const handler = (await import('~~/server/api/leads/email-endpoints/index.get')).default
    await handler({
      context: {},
      query: { client_id: '11111111-1111-4111-8111-111111111111' }
    } as never)
    expect(listEmailEndpoints).toHaveBeenCalledWith(
