...
vi.mock('~~/server/utils/pageStudio/machineAuth', () => ({
  requirePageStudioMachineAuth: mocks.auth
}))
...
describe('private collection upgrade admission endpoint', () => {
  ...
  it('denies before reading an unauthenticated request body', async () => {
    ...
    const { default: handler }
      = await import('~~/server/routes/internal/page-studio/collection-staging-upgrades/authorize.post')
      ...
      context: {
        cloudflare: {
          env: {
            ...
            PAGE_STUDIO_PROVISIONER: { createProvisioning: vi.fn(), readProvisioning: vi.fn() }
          }
        }
      }
    }
