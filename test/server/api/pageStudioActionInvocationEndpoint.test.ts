...
vi.mock('~~/server/utils/pageStudio/actionCoordinator', async original => ({
  ...(await original<object>()),
  coordinateActionInvocation: mocks.update
}))
...
describe('native action invocation endpoint', () => {
  const event = {
    context: {
      cloudflare: { env: { PAGE_STUDIO_CONTENT_ENVIRONMENT: 'staging' } }
    }
  }
  const call = async () =>
    (
      await import('~~/server/routes/internal/page-studio/action-invocations.post')
    ).default(event as never)
  beforeEach(() => {
    vi.resetAllMocks()
    mocks.header.mockImplementation((_event, key) =>
      key === 'x-page-studio-session'
        ? 'signed-token'
        : key === 'content-type'
          ? 'application/json'
          : undefined
  ...
  it('forwards only verified identity and bound environment with no-store', async () => {
    ...
    expect(mocks.update).toHaveBeenCalledWith(body, {
      source: 'studio-session',
      claims: { nonce: 'native-identity' },
  ...
  it('rejects oversized streamed bytes even without Content-Length', async () => {
    mocks.stream.mockImplementation(() => stream(' '.repeat(131073)))
  ...
  it.each([
    ...
    { ...body, userId: 'forged' }
  ])('rejects authored authority or amounts %#', async (value) => {
