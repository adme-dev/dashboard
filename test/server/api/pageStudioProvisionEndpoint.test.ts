...
describe('portal Page Studio provisioning handoff', () => {
  ...
  it('emits a stable scoped request only for an accepted revision', async () => {
    mocks.queryOneFresh.mockResolvedValue({ tenantId: 't1', clientId: 'c1', siteId: 's1', revision: 1, status: 'accepted', source: 'chat', plan: { pages: ['home'], templateId: 'limousine-v1' } })
  ...
  it('rejects a proposal that has not been accepted', async () => {
    mocks.queryOneFresh.mockResolvedValue({ tenantId: 't1', clientId: 'c1', siteId: 's1', revision: 1, status: 'proposed', source: 'template', plan: {} })
    const { default: handler } = await import('~~/server/api/portal/page-studio/sites/[siteId]/provision.post')
  ...
  it('fails explicitly when the provisioner binding is unavailable', async () => {
    mocks.queryOneFresh.mockResolvedValue({ tenantId: 't1', clientId: 'c1', siteId: 's1', revision: 1, status: 'accepted', source: 'template', plan: { templateId: 'retail-v1' } })
    const { default: handler } = await import('~~/server/api/portal/page-studio/sites/[siteId]/provision.post')
  ...
  it('requires fresh editor membership before starting resources for a site', async () => {
    ...
    const { default: handler } = await import('~~/server/api/portal/page-studio/sites/[siteId]/provision.post')
    await expect(handler({ id: 's1', body: { expectedRevision: 1 }, context: { cloudflare: { env: { PAGE_STUDIO_PROVISIONER: binding } } } } as never))
    ...
    expect(params).toEqual(['c1', 's1', 1, 'u1'])
  })
