...
vi.mock('~~/server/utils/pageStudio/collectionUpgradeAuthority', () => ({ authorizePageStudioCollectionUpgrade: mocks.authorize }))
...
describe('private collection upgrade admission endpoint', () => {
  ...
  it('denies before reading an unauthenticated request body', async () => {
    ...
    const { default: handler } = await import('~~/server/routes/internal/page-studio/collection-upgrades/authorize.post')
