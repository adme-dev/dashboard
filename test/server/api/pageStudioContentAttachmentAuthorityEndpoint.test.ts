...
vi.mock('~~/server/utils/pageStudio/contentAttachmentAuthority', () => ({ authorizePageStudioContentAttachment: mocks.authorize }))
...
describe('internal CMS attachment authority endpoint', () => {
  ...
  it('authenticates before reading user input or accessing authority', async () => {
    ...
    const { default: handler } = await import('~~/server/routes/internal/page-studio/content-attachments/authorize.post')
  ...
  it('uses only the trusted binding and prevents caching of the authority result', async () => {
    ...
    expect(mocks.authorize).toHaveBeenCalledWith(body, 'staging')
  })
})
