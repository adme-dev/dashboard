...
describe('CRM search preview binding isolation', () => {
  ...
  it('keeps the checked-in Pages production and preview environments explicit', () => {
    ...
    expect(previewBindings).toContain('binding = "PAGE_STUDIO_CONTENT_ROUTER"')
    expect(previewBindings).toContain('service = "xeroflow-content-router-staging"')
    expect(previewVars).toContain('PAGE_STUDIO_CONTENT_ENVIRONMENT = "staging"')
    expect(previewBindings).toContain('binding = "PAGE_STUDIO_PROVISIONER"')
    expect(previewBindings).toContain('service = "xeroflow-provisioning-staging"')
    expect(previewVars).toContain('PAGE_STUDIO_PROVISIONING_ENVIRONMENT = "staging"')
    expect(previewBindings).toContain('[[env.preview.services]]\nbinding = "PAGE_STUDIO_MANAGEMENT"\nservice = "xeroflow-page-studio-management-staging"')
    expect(previewBindings).not.toContain('xeroflow-page-studio-management-production')
    expect(previewBindings.match(/\[\[env\.preview\.services\]\]/gu)).toHaveLength(5)
    expect(previewBindings.match(/^service = /gmu)).toHaveLength(5)
