...
describe('Persona intent tiers migration 311', () => {
  it('adds tier_rank, a tier-membership table, and seeds 3 ranked tier definitions', () => {
    ...
    expect(migration).toMatch(/ALTER TABLE crm_persona_definitions\s+ADD COLUMN IF NOT EXISTS tier_rank INTEGER NULL;/)
    expect(migration).toContain('CREATE TABLE IF NOT EXISTS crm_persona_tier_memberships')
    ...
    expect(migration).toContain('CREATE INDEX IF NOT EXISTS idx_crm_persona_tier_memberships_tier')
