...
describe('God mode gate inventory', () => {
  ...
  it('freezes every pre-existing direct gate with an explicit classification', () => {
    ...
    expect(inventory.rows).toHaveLength(1506)
    expect(inventory.counts).toEqual({
      identity_tenant_hard_boundary: 104,
      provider_infrastructure_availability: 222,
      application_governance_bypass: 1590,
      ordinary_user_behavior: 171,
      unrelated_configuration: 418
    })
    expect(inventory.digest).toBe('deb211a75cdf569e9dc0ca99ddb37550e6e2c16775a68a72a89e0ca967d1e171')
