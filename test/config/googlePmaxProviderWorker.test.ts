...
interface WranglerConfig {
  ...
  workers_dev?: boolean
  env?: {
    production?: { services?: Array<Record<string, unknown>> }
    preview?: { services?: Array<Record<string, unknown>> }
  }
...
describe('Google PMax provider Worker deployment boundary', () => {
  it('is private and bound to the Pages application', () => {
    ...
    expect(pages.env?.production?.services).toContainEqual({
      binding: 'GOOGLE_PMAX_PROVIDER',
      service: 'google-pmax-provider'
    })
    expect(pages.env?.preview?.services ?? []).not.toContainEqual(
      expect.objectContaining({ binding: 'GOOGLE_PMAX_PROVIDER' })
    )
  })
