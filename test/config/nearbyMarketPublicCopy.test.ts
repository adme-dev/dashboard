...
describe('nearby market legal and product copy', () => {
  ...
  it('documents dormant pilot controls and all operational gates', () => {
    ...
    expect(runbook).toMatch(/stable\s+controlled-egress/i)
    expect(runbook).toMatch(/IP addresses[\s\S]*\/32/i)
    expect(runbook).toMatch(/hard no-go/i)
    expect(runbook).toMatch(/readiness booleans do not prove/i)
