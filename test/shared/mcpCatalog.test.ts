...
describe('MCP catalog rollback guard', () => {
  ...
  it('rejects a release or tool-count regression', () => {
    expect(MCP_CATALOG_RELEASE).toBe('2026-08-28.14')
    expect(MCP_PREVIOUS_CATALOG_RELEASE).toBe('2026-08-24.13')
    expect(MCP_SERVER_VERSION).toBe('1.0.4')
    expect(MCP_MIN_TOOL_COUNT).toBe(95)
    expect(() => assertMcpCatalogNotRegressed('2026-08-24.13', 95)).toThrow('MCP catalog regression')
    expect(() => assertMcpCatalogNotRegressed('2026-08-28.14', 94)).toThrow('MCP catalog regression')
    expect(() => assertMcpCatalogNotRegressed('2026-08-28.14', 95)).not.toThrow()
  })
})
