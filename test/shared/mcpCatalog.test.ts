...
describe('MCP catalog rollback guard', () => {
  ...
  it('rejects a release or tool-count regression', () => {
    expect(MCP_CATALOG_RELEASE).toBe('2026-09-03.1')
    expect(MCP_PREVIOUS_CATALOG_RELEASE).toBe('2026-09-01.15')
    expect(MCP_SERVER_VERSION).toBe('1.0.4')
    expect(MCP_MIN_TOOL_COUNT).toBe(164)
    expect(() => assertMcpCatalogNotRegressed('2026-09-01.15', 164)).toThrow('MCP catalog regression')
    expect(() => assertMcpCatalogNotRegressed('2026-09-03.1', 163)).toThrow('MCP catalog regression')
    expect(() => assertMcpCatalogNotRegressed('2026-09-03.1', 164)).not.toThrow()
  })
})
