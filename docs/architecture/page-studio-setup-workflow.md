...
Only `env.preview` binds `PAGE_STUDIO_PROVISIONER` to `xeroflow-provisioning-staging`, with explicit provisioning environment `staging`. Both preview database bindings remain the dedicated cache-disabled Hyperdrive `3865ea5568234fc7b0e9e3e595a30286` for Neon branch `staging/page-studio`. There is no root or production provisioner binding. The staging coordinator scheduler is now enabled at Foundation source `b0466fe`, verified Worker version `4bba954b-a615-4683-bf20-aac5d018fd85`. Positive two-customer provisioning completed, including an automatically scheduled second customer; their routes and sessions were retired afterwards. The separate Foundation evidence is `docs/research/evidence/2026-09-14-scheduled-customer-provisioning.json`. This establishes staging execution only; production provisioning remains unbound and gated.
...
Local checks: 66 focused tests pass, including four real PostgreSQL authorization tests. CI explicitly runs the database suite. Desktop/mobile Chrome checks with synthetic intercepted API responses verify scrolling, no page-width overflow, whole-cent conversion, identical quote retry and no page errors. The initial build was below the unchanged raw Worker limit (25,458,821 / 25,468,928 bytes). PR546 is now merged as `7b1f0edf5509dceb73d18f1a2cb69a4f5990af4f`; live preview deployment `232000c3` from source `2071657` passed real 1440×900 and 393×650 browser quotes, approvals, replay, stale-write and tenant-isolation acceptance. These synthetic staging fixtures were retired with zero remaining sessions. Evidence is recorded in the separate Foundation repository at `docs/research/evidence/2026-09-14-dashboard-booking-operations.json`. Production remains unbound; this does not establish live customer booking operations. No migration is required.

## Business content administration

Client website cards now link to a scoped business-content editor for agency and portal users where private content storage is connected. Collections cover services, fleet, testimonials and other business records. Editors can maintain names, descriptions, review status and typed details; viewers cannot edit. Every operation freshly verifies the active client, entitlement and exact membership. Saves use expected revisions and exact receipts; conflicting edits stay local until an explicit reload. Saving does not publish a website. See `page-studio-business-content-admin.md` for the boundary and verification scope.
