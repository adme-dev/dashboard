...
- Pages app commit: `ba2aa4c43`; deployment `278c7ef8.agency-dashboard-6cm.pages.dev` on production branch `main`. Sync errors remain visible while the connected-location count and retry action stay available.
...
- Final repeat-sync verification: Mornington advanced from 68 to 100 reviews and changed its historical cursor in a successful 47-second production run, with no timeout or review error. A subsequent storage audit found 524 Google reviews, all 57 unanswered low-star reviews urgent, and zero duplicate conversations. Temporary-table SQL verification confirmed edits update content without changing counters or queuing automation.
- Live AI drafting initially failed with HTTP 404 because `llama-3.3-70b-versatile` was retired for developer-tier usage on 2026-08-16 ([Groq notice](https://console.groq.com/docs/deprecations)). Saved an audited Model Ops override for **only** `social_inbox_reply_draft` to `openai/gpt-oss-120b`. The live composer then generated a personalised Google reply; the invocation ledger confirms success at 05:57:37 UTC. The test draft was cleared without sending. Keep this override: resetting it restores the retired source default. Other AI feature defaults were outside this Google-review repair.
