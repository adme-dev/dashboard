...
- [x] **W-2 — observed-memory distiller** *(built, dormant)*: `createWorkEventSource` (`server/utils/ai/observe/source.ts`) over task_activities / crm_activities / proof_activities / ai_action_audit — each read STRICTLY actor-scoped; `client_activity_log` excluded (client actor, no staff owner); sensitive kinds (sign-off/approval/finance) flagged via `isSensitiveKind` so `detectRoutines` drops them. Pure observed-distiller (`distill.ts`: prompt + tolerant parser + injected gpt-oss-20b). Per-user orchestrator (`run.ts`: `observeUser`/`runObservePass`) → `sessionize`→`detectRoutines`→`distillObserved`→`upsertMemory(source='observed', scope='user')`. Watermark table `ai_observe_state` (mig **188**, applied). Cron `server/api/cron/observe-and-learn.post.ts` (x-cron-secret + `AI_OBSERVE_ENABLED` gate) + companion Worker `workers/observe-cron/` (daily 06:20 UTC). 27 unit tests; lint + server-tsc clean; reviewed (no Critical/High — fixed Low watermark boundary-skip + weekly-routine under-detection by reading a FIXED rolling lookback window each run, watermark used only to skip no-change users). **Substrate note:** built on the proven cron+Worker rails (not Cloudflare Workflows — none exist in this codebase yet); the per-user watermark + idempotent upsert give the durability/resumability the Workflow framing wanted, deployably and unit-testably. **Substrate is decoupled** — a Workflow wrapper can replace the cron later without touching the pure orchestrator. Team-wide routines surfacing as DS-2 proposals is deferred to a follow-up. W-1 pure core (`751dab27`) reused unchanged.
- [x] **W-3 — transparency panel** *(built)*: "What I've learned from your work" card on `/agency/ai/my-assistant` — lists the caller's own `source='observed'` memories (deletable, right-to-forget) + a read-only "Team & agency knowledge" section of the department/org memories visible to them. New store fns `listUserMemoriesBySource` + `deleteMemoryById` (both user_id-isolated; foreign id → 404, no info leak). Endpoints `GET /api/agency/ai/my-assistant/memories` + `DELETE …/memories/[id]`. Confirm-before-forget UModal + toast. 4 new store tests (511 AI tests green); lint/server-tsc clean. Not gated (read/delete of one's own memory is always safe); the panel simply shows nothing until W-2 (or `remember`) writes rows.
