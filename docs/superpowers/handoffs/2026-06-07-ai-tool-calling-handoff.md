...
- **Loop model (DECIDED 2026-06-07 — Option 2: Groq open-source default):** primary **Groq `gpt-oss-120b`**, fallback **Kimi K2** (`groq/moonshotai/kimi-k2-instruct`) — both via gateway `/groq`; **runnable + bake-off-able LOCALLY today** (`GROQ_API_KEY` is set; local env has no `ANTHROPIC_API_KEY`/`AI_GATEWAY_URL`). **Claude Sonnet 4.6 kept as a dormant escape hatch** — `@ai-sdk/anthropic` stays installed but activates only with `ANTHROPIC_API_KEY` + gateway in prod, for A/B or if the Groq bake-off shows insufficient tool reliability on our real tools. **Bake-off (Plan Phase 8) now runs locally on Groq** (gpt-oss-120b vs Kimi K2 vs gpt-oss-20b) → winner = primary. Full Option 1 (drop Anthropic) later = `pnpm remove @ai-sdk/anthropic` + remove the fallback spec.
