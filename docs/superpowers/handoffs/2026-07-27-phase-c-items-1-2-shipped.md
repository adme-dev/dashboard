...
- **Confirmed by a brief follow-up check** (a fresh worktree, `EnterWorktree` → `phase-c-exclusion-audiences`, no code changes made, worktree already cleaned up): `competitive_referrer` and `exit_intent` are real discrete tracked events (`public/track.js`), so they fit the existing `positive_signals`/`negative_signals` signal-key matching model directly. But **`bounce <3s` is NOT a discrete signal** — grepped `public/track.js` and `signalLedger.ts`, no `bounce` event exists anywhere. It would have to be derived from session duration/engagement data, the same class of gap as intent-tier scoring's deferred "VDP dwell time" (numeric threshold, not binary signal presence) — the existing `scorePersonaDefinition` engine can't express it as-is. Worth deciding early in the item-3 brainstorm whether exclusion audiences ship v1 with `competitive_referrer`/`exit_intent` only (binary-only, ships fast, consistent with how intent-tier scoring scoped itself) or also tackle the bounce-duration threshold work.
