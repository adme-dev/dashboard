...
**Workflow Oracle / supervisor (control plane over all jobs) — deferred to the multi-agent tier; NOT in Slice 1.** A meta-orchestrator that *oversees the whole job/agent fleet* rather than answering one chat turn: a job/agent registry + live status, heartbeat scheduling, routing/delegation to functional agents (marketing/finance/sales), cost/budget governance + throttling, approval gating (`route_for_approval`), failure/retry oversight, and unified audit/observability. This is the Paperclip "company of agents" control plane — distinct from Slice 1's single-assistant loop. It's the natural home for BOTH the future AI agent fleet AND oversight of our existing background-job fleet (crons, companion Workers, queues — anomaly detection, social dispatch, ga4-sync, audio-jobs, etc.). **Strong precursor already exists** — the AI Agent / proactive-digest engine: `server/utils/aiAgentRunner.ts` (`runAgentDigest`) + `aiAgentAnalyzer.ts`'s 11 business-wide analyzers (`runAllAnalyzers`: overdue/stale-briefs/blocked/deadline-risk/ad-spend/financial/EOM/workload/unassigned/timesheet) + `ai_agent_runs` audit (mig 015) + a trigger/preferences/runs/reports surface under `/api/agency/ai/agent/*`. That already delivers proactive oversight + run-tracking. The Oracle slice = **evolve this engine into a control plane** (add a job/agent registry over the broader fleet — crons, companion Workers, queues — plus routing/delegation to functional agents, cost/budget governance, and approval-gated actions via `route_for_approval`) — **not greenfield**. Re-evaluate Flue (durable execution/subagents) here.
