...
1. **`aformat=sample_rates=<state.sample_rate>:channel_layouts=stereo` FIRST** — normalise every input's rate + layout *before* it reaches any `amix`. This is the **#1 silent `amix` failure** in the OSS prior-art checklist (`oss-prior-art.md` §2/§4: "aformat every input before amix"); clips of mixed rate/layout otherwise mix to garbage. Non-negotiable, applied at source so both the track-level and final `amix` are safe.
2. `atrim=start=<source_in_sec>:end=<source_out_sec>` (omit `end` when `source_out_sec` is null → play to source end), then `asetpts=N/SR/TB` (reset timestamps after trim).
3. `adelay=<timeline_start_sec*1000>:all=1` to position the clip on the timeline (`:all=1` = apply to all channels regardless of count — the prior-art `adelay`-is-per-channel trap; gaps are implicit silence before `adelay`).
4. `volume=<gain_db>dB` for clip gain (skip when 0).
5. `afade=t=in:st=0:d=<fade_in_sec>` and `afade=t=out:st=<clipPlayLen-fade_out_sec>:d=<fade_out_sec>` when non-zero. `fade_curve` maps to ffmpeg `curve=` (`linear`→`tri`, `exp`→`exp`, `log`→`log`); exact curve token table pinned in tests.

Per **track**:
6. `amix=inputs=N:normalize=0:duration=longest` the track's clips into one track-bus stream (`duration=longest` chosen deliberately — clips are positioned by `adelay`, so the bus must span to the last clip's end, not truncate to the first); apply `volume=<track.gain_db>dB`. A `muted` track is **dropped from the mix entirely** (not silenced) — cheaper and identical output. A single-clip, zero-gain track reuses the clip label directly (no extra node).

**Ducking** (declarative → `sidechaincompress`):
7. For each `DuckingRule`, the `target_track_id` bus is compressed keyed by the `source_track_id` bus: `sidechaincompress=threshold=<threshold_db>:ratio=<derived>:attack=<attack_ms>:release=<release_ms>`. The source bus is `asplit` so it both triggers ducking and remains in the final mix. (Web Audio has no sidechain; SP2 will compile the *same* rule to scheduled gain ramps — see SP0 §4. SP1 owns the ffmpeg compilation; the rule is the single source of truth. `attack`/`release` defaults of 50/300 ms avoid the prior-art "pumping" trap.)

Final:
8. `amix=inputs=<numTrackBuses>:normalize=0:duration=longest` all (post-duck) track buses, then **`alimiter=limit=0.95`** → `[mix]`. The `alimiter` is the prior-art "avoid post-mix clipping" safety: summing buses with `normalize=0` can exceed 0 dBFS and hard-clip the `pcm_s16le` WAV *before* the per-channel `loudnorm` ever sees it. Output a full-quality master **WAV** at `state.sample_rate` (no loudnorm here — normalisation is the per-channel pass). `alimiter` is applied even for a single surviving bus, so the master is always clip-safe.
...
- **Container** (`RenderContainer`): a new `POST /render-timeline` endpoint in `container/server.mjs` that (a) receives the clip bytes + filtergraph plan, (b) runs ffmpeg for the master via the ported builder (kept in sync with the TS source, per the existing render.mjs convention), (c) returns the master WAV bytes. The Worker then reuses the **existing** `renderVariants` per-channel pipeline (`/render` endpoint) off that master.
- **Long-render heartbeat (prior-art lifecycle):** `RenderContainer` has `sleepAfter='5m'`; a multi-clip master render plus N loudnorm passes can exceed that and be killed mid-render. Per the Kent C. Dodds CF-ffmpeg lifecycle (`oss-prior-art.md` §2), the Worker must **`renewActivityTimeout()` (heartbeat) around each Container call** so `sleepAfter` cannot reap an in-flight render. The synchronous `fetch` + `AbortSignal.timeout` already used by `renderVariants` bounds the upper limit; the heartbeat keeps the instance alive up to it.
- **Idempotency / retries:** the queue may redeliver. The worker treats a job already `done` as a no-op, and re-renders cleanly into the same `media/<projectId>/<jobId>/…` keyspace (overwrite-safe) on retry of a `rendering`/`failed` job. Retry-in-place, no local fallback (the prior-art "fallback paradox").
...
- **Render duration / Container limits.** Multi-clip downloads + mix + N loudnorm passes could be long; the queue (durable, retried) is the right transport, and the §5 `renewActivityTimeout()` heartbeat prevents `sleepAfter` reaping an in-flight render, but Container CPU/time limits should still be checked against worst-case ad-length timelines during implementation.
- **ffmpeg edge cases adopted from prior art (`oss-prior-art.md` §2/§4).** Beyond the three folded into §3 (`aformat`-before-`amix`, `amix duration=longest` + `alimiter`, the `renewActivityTimeout` heartbeat), the implementation should also heed the carried prior-art notes that live in the *reused* `render.ts`/container, not new SP1 code: `loudnorm` two-pass with `linear=true` (already in `render.ts`), its silent dynamic-AGC fallback under tight headroom (log it), the 192 kHz internal resample (pin output `-ar` — the master argv does via `state.sample_rate`), and the VBR duration-header trap on mp3 variants (a `render.ts` concern). These are noted for the implementer's awareness; SP1 does not modify `render.ts`.
