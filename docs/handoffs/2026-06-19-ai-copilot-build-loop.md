...
- [~] **Self-service config — Personalize tier** (`5e88ccfa`) — `ai_agent_configs` (mig 183, applied) + `agentConfig.ts` (`narrowToolsByConfig` pure: config SUBTRACTS only, never grants — applied last in `runToolLoop`) + memory toggle + GET/PUT `/api/agency/ai/my-assistant`, wired into the engine (L1 + L2). 6 tests. **Remaining:** the "My Assistant" settings UI page + the Build/Govern tiers (shared mini-skill-packs, kill-switch).
...
- **Virtual Office Mode A — room-scoped context built** (`ad40b6fa`, no migration) — engine-level: room-scoped `ToolContext` + membership-gated prompt enrichment, dormant behind `AI_TOOLS_ENABLED`.
- **Portal Tier 1 — isolation foundation built** (`538776f4`, no migration) — separate clientScope-required portal registry + 5 read tools + cross-tenant fuzz gate; no live loop yet. Head now `538776f4`; AI suite **374/374 green**, zero new type errors.
- **Remaining Phase 3:** Mode A office-page dock UI (+ Mode C/B later), Portal Tier 1 live loop+endpoint+`PortalCopilot.vue`+more tools+app-config (+Tier 2), the "My Assistant" config UI + Build/Govern tiers. *(UNREVIEWED commits to fold into the next `/code-review high`: self-service-config `5e88ccfa`, Mode A `ad40b6fa`, Portal Tier 1 `538776f4`.)*
