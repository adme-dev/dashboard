...
- **Virtual Office Mode A — room-scoped context built** (`ad40b6fa`, no migration) — engine-level: room-scoped `ToolContext` + membership-gated prompt enrichment, dormant behind `AI_TOOLS_ENABLED`. Head now `ad40b6fa`; AI suite **366/366 green**, zero new type errors.
- **Remaining Phase 3:** Mode A office-page dock UI (+ Mode C/B later), Portal Tier 1, the "My Assistant" config UI + Build/Govern tiers. *(The self-service-config `5e88ccfa` AND Mode A `ad40b6fa` are UNREVIEWED — fold both into the next `/code-review high`.)*
