  ...
  'page-studio': {
    ...
    details: [
      ...
      },
      {
        ...
        content: 'The website workspace shows the pages, SEO and form counts from your saved Studio website. Open Studio to edit the same draft, then return to see the latest saved changes. Build a complete page tree with nested subpages, responsive desktop, tablet, and mobile previews, page visibility, shell inheritance, and per-page SEO. Compact preset galleries add canonical sections, page layouts, headers, navigation, mobile menus, footers, themes, and complete site starters as ordinary editable content. AI proposals use the same typed components and controlled operations as manual editing, while revision-safe checkpoints prevent one session from silently overwriting another. Draft history lets authorised editors name a saved version and restore earlier saved work as a new draft, preserving previous checkpoints and the live website. Preview-only edits must be saved through supported controls to appear in history.'
      },
      {
      ...
      },
      {
        ...
        content: 'Drafts, editor sessions, checkpoints, reviews, approvals, server-owned builds, and immutable releases are separated so editing does not silently become production. Agency editors can name the current saved draft and submit it for review in one action; ordinary named versions remain drafts. The launch checklist brings saved content, current approval, enquiry forms, domain and HTTPS status, email setup and website access together. Reviewers compare saved page, component, form and site changes with a named live release, then approve, reject or return the version to draft. Changed checkpoints and release pointers require a fresh comparison. Publishing checks the exact saved version and current release again before proceeding. Where approved-build review is enabled, publishers prepare the website, open its authenticated preview, confirm their review and publish that exact build. Reopening review recovers the retained build, and short-lived preview access ends when the originating login or publishing access is revoked. Saved form definitions and email preferences remain distinct from verified receipt and delivery tests. XeroFlow verifies the approved R2 checkpoint and synchronises navigation, footer, theme, SEO, and integrations with the activated release.'
      },
      {
        title: 'A Staging Address for Each Website',
        content: 'Preview the saved website at its own stable XeroFlow address before connecting a custom domain. Agency editors and eligible client editors can update staging from the website workspace; viewers can open a ready preview. The workspace distinguishes an assigned address from a verified deployment and shows when the saved draft has changed. A failed update keeps the previous successful snapshot available. Staging previews exclude private pages, carry a visible preview banner and discourage search indexing. Form submissions are disabled in this preview rollout. Staging and release builds share the website package allowance; retrying the same accepted build request does not consume another build.'
      },
      {
      ...
      },
      {
        ...
        content: 'Staff can prepare and revise a website setup plan from a starter or brief, review its pages and modules, and keep unconfirmed business details visible before launch. Eligible customer administrators and managers can create a website from a business template or brief. The draft, editor access and initial review proposal are saved together only when subscription limits allow them. Customers with website editing access can submit business details and revised plans from their portal; viewers can read the plan and progress. Agency approval is tied to the current revision. Where the setup service is connected, staff and authorised customer editors can explicitly request preparation of the approved revision and track its progress. Authorised agency staff can record a client’s website access period, permitted modules and limits for sites, pages, storage, domains, AI operations, builds and traffic. Trial access requires an expiry, while client website creation can be enabled separately. Each grant records its reason and actor in the billing audit; matching retries return the original result and competing grants cannot replace existing access. These access grants do not create charges or a paid billing subscription. Renewal, plan changes and provider billing remain separate rollout work.'
      }
    ]
  },
