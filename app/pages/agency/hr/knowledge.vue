...
type PromotionCandidate = { source_type: 'published_finding' | 'completed_action'; source_id: string; title: string; suggested_content: string; source_date: string; limitations: string[] }
...
const promotionCandidates = ref<PromotionCandidate[]>([])
const selectedCandidate = ref<PromotionCandidate | null>(null)
const promotionForm = reactive({ title: '', content: '', provenanceNote: '', limitations: '' })
...
async function refresh() {
  loading.value = true
  try { const [data, promotionData] = await Promise.all([apiFetch<KnowledgeResponse>('/api/agency/hr/knowledge'), apiFetch<{ candidates: PromotionCandidate[] }>('/api/agency/hr/knowledge/promotion-candidates')]); entries.value = data.entries; policy.value = data.policy; promotionCandidates.value = promotionData.candidates }
...
function reviewPromotion(candidate: PromotionCandidate) {
  selectedCandidate.value = candidate
  promotionForm.title = candidate.title
  promotionForm.content = candidate.suggested_content || ''
  promotionForm.provenanceNote = `Drafted from the authorised ${candidate.source_type.replaceAll('_', ' ')} record. A human reviewer must verify transferability, remove unnecessary personal detail and approve any future version.`
  promotionForm.limitations = candidate.limitations.join('\n')
}
async function createPromotionDraft() {
  if (!selectedCandidate.value) return
  saving.value = true
  try {
    await apiFetch('/api/agency/hr/knowledge/promote', { method: 'POST', body: {
      sourceType: selectedCandidate.value.source_type, sourceId: selectedCandidate.value.source_id,
      title: promotionForm.title, content: promotionForm.content,
      provenanceNote: promotionForm.provenanceNote, limitations: lines(promotionForm.limitations),
    } })
    toast.add({ title: 'Governed knowledge draft created', description: 'The source-cited draft remains excluded from established knowledge and general AI until separately reviewed.', color: 'success' })
    selectedCandidate.value = null
    await refresh()
  } catch (error: any) { toast.add({ title: 'Learning was not promoted', description: error?.data?.statusMessage || 'Review the source and draft.', color: 'error' }) }
...
<template>
  <div class="min-h-full bg-default">
    ...
    <main class="mx-auto max-w-7xl space-y-6 px-5 py-8 sm:px-8">
      ...
      <section class="overflow-hidden rounded-xl border border-default bg-default" aria-labelledby="learning-candidates-heading"><div class="flex flex-col gap-3 border-b border-default px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p class="font-mono text-xs uppercase tracking-[0.16em] text-muted">Manual promotion queue</p><h2 id="learning-candidates-heading" class="mt-1 text-lg font-semibold text-highlighted">Review learning candidates</h2><p class="mt-1 text-sm text-muted">Published findings and completed actions can become source-cited drafts. Nothing is promoted automatically.</p></div><UBadge color="neutral" variant="subtle" :label="`${promotionCandidates.length} candidates`" /></div><div v-if="promotionCandidates.length" class="max-h-72 divide-y divide-default overflow-y-auto overscroll-contain"><article v-for="candidate in promotionCandidates" :key="`${candidate.source_type}:${candidate.source_id}`" class="grid gap-3 px-5 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div><div class="flex flex-wrap items-center gap-2"><p class="text-sm font-medium text-highlighted">{{ candidate.title }}</p><UBadge color="neutral" variant="outline" :label="candidate.source_type.replaceAll('_', ' ')" /></div><p class="mt-1 line-clamp-2 text-xs leading-5 text-muted">{{ candidate.suggested_content }}</p></div><UButton color="neutral" variant="outline" size="sm" label="Review candidate" @click="reviewPromotion(candidate)" /></article></div><div v-else class="px-5 py-7 text-center text-sm text-muted">No published findings or completed actions are awaiting knowledge review.</div></section>
      ...
      <UModal :open="Boolean(selectedCandidate)" title="Create governed draft" description="Review and minimise the learning before creating a restricted HR knowledge draft." @update:open="open => { if (!open) selectedCandidate = null }"><template #content><div class="flex max-h-[calc(100vh-96px)] flex-col overflow-hidden"><div class="border-b border-default px-6 py-5"><p class="font-mono text-xs uppercase tracking-[0.16em] text-primary">Manual review learning</p><h2 class="mt-1 text-xl font-semibold text-highlighted">Create governed draft</h2><p class="mt-2 text-sm text-muted">Nothing is promoted automatically. Remove names, unnecessary personal detail and claims that are not transferable.</p></div><div v-if="selectedCandidate" class="space-y-5 overflow-y-auto p-6"><UFormField label="Draft title" required><UInput v-model="promotionForm.title" class="w-full" /></UFormField><UFormField label="Minimum reusable learning" required><UTextarea v-model="promotionForm.content" :rows="7" class="w-full" /></UFormField><UFormField label="Provenance note" required><UTextarea v-model="promotionForm.provenanceNote" :rows="4" class="w-full" /></UFormField><UFormField label="Limitations" required help="One limitation per line."><UTextarea v-model="promotionForm.limitations" :rows="4" class="w-full" /></UFormField><UAlert color="warning" variant="soft" icon="i-lucide-book-lock" title="Draft only" description="This action does not approve the learning, index it, or expose it to general AI. Approval remains a separate human decision." /></div><div class="flex justify-end gap-2 border-t border-default p-4"><UButton color="neutral" variant="ghost" label="Cancel" @click="selectedCandidate = null" /><UButton icon="i-lucide-file-plus-2" label="Create governed draft" :loading="saving" @click="createPromotionDraft" /></div></div></template></UModal>
    </main>
