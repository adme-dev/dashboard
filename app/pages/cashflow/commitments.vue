...
import { parseDate, getLocalTimeZone, type CalendarDate, type DateValue } from '@internationalized/date'
...
async function seedStatutory() {
  ...
  try {
    ...
    toast.add({ title: 'Seeding failed', description: err?.data?.statusMessage ?? err?.message, color: 'error' })
  } finally {
    seeding.value = false
  }
}
...
<template>
  <div class="p-6 space-y-6">
      ...
      <template #description>
        <div class="space-y-2 mt-1">
          ...
          >
            ...
            <UButton size="xs" variant="soft" @click="confirmMatch(m)">
              Confirm match
            </UButton>
          </div>
        </div>
    ...
    <UCard>
      ...
      <div v-if="pending" class="py-12 text-center text-muted text-sm">
        Loading commitments…
      </div>
      <div v-else-if="!commitments.length" class="py-12 text-center">
        ...
        <p class="text-sm text-muted mt-1">
          Add a payment you expect before its invoice exists in Xero.
        </p>
      </div>
          ...
          <tbody>
            ...
            >
              <td class="py-2.5 pr-4">
                ...
                <div v-if="c.description" class="text-xs text-muted truncate max-w-[28ch]">
                  {{ c.description }}
                </div>
              </td>
              <td class="py-2.5 pr-4 text-right tabular-nums">
                {{ fmtAmount(c.amount_cents) }}
              </td>
              ...
              <td class="py-2.5 pr-4 whitespace-nowrap">
                {{ ACCOUNT_LABELS[c.payment_account] ?? c.payment_account }}
              </td>
              <td class="py-2.5 pr-4">
      ...
      <template #content>
        <div class="p-6 space-y-4">
          <h2 class="text-lg font-semibold">
            {{ editing ? 'Edit commitment' : 'Add commitment' }}
          </h2>
          ...
          <UFormField label="Notes">
            <UTextarea
              v-model="form.notes"
              :rows="3"
              placeholder="Assumptions, hold reasons, follow-ups"
              class="w-full"
            />
          </UFormField>

          <div class="flex justify-end gap-2 pt-2">
            <UButton variant="ghost" color="neutral" @click="showForm = false">
              Cancel
            </UButton>
