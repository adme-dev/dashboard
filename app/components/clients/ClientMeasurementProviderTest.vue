...
const props = defineProps<{
  ...
  destination: Pick<MeasurementDestination, 'id' | 'platform' | 'mappings'>
}>()
        ...
        body: {
          ...
          ...(isMeta
            ? {
                ...
                browserEventId: browserEventId.value.trim() || null
              }
            : {
...
<template>
  <section class="mt-4 rounded-lg border border-primary/25 bg-primary/5 p-4" data-testid="measurement-provider-test">
    <div class="flex items-start justify-between gap-4">
      ...
      <UButton label="Close" color="neutral" variant="ghost" size="xs" :disabled="pending" @click="emit('close')" />
    </div>
    ...
    <div class="mt-4 grid gap-4 sm:grid-cols-2">
      ...
      <template v-if="destination.platform === 'meta'">
        <label class="space-y-1.5 text-sm">
          ...
          <input v-model="testEventCode" data-testid="provider-test-code" autocomplete="off" maxlength="128" class="w-full rounded-md border border-default bg-default px-3 py-2 font-mono text-sm">
        </label>
        <label class="space-y-1.5 text-sm">
          <span class="font-medium text-highlighted">Approved Meta lead ID</span>
          <input v-model="metaLeadId" data-testid="provider-test-meta-lead-id" inputmode="numeric" autocomplete="off" maxlength="16" class="w-full rounded-md border border-default bg-default px-3 py-2 font-mono text-sm">
        </label>
        <label class="space-y-1.5 text-sm">
          <span class="font-medium text-highlighted">Shared browser event ID</span>
          <input v-model="browserEventId" data-testid="provider-test-browser-event-id" autocomplete="off" maxlength="128" class="w-full rounded-md border border-default bg-default px-3 py-2 font-mono text-sm">
        </label>
      </template>
      ...
      <template v-else>
        ...
        <label class="space-y-1.5 text-sm">
          ...
          <input v-model="clickValue" data-testid="provider-test-click-id" autocomplete="off" maxlength="512" class="w-full rounded-md border border-default bg-default px-3 py-2 font-mono text-sm">
        </label>
      </template>

      <label class="space-y-1.5 text-sm sm:col-span-2">
        <span class="font-medium text-highlighted">Approval reason</span>
        <textarea v-model="reason" data-testid="provider-test-reason" rows="2" maxlength="1000" class="w-full resize-y rounded-md border border-default bg-default px-3 py-2 text-sm" />
      </label>
    </div>

    <label class="mt-4 flex items-start gap-3 rounded-md border border-warning/25 bg-warning/5 p-3 text-sm">
      <input v-model="confirmed" data-testid="provider-test-confirmed" type="checkbox" class="mt-0.5 size-4 rounded border-default">
    ...
    <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p v-else-if="result?.status === 'accepted'" role="status" class="text-sm text-success">Provider accepted the test request.</p>
        <p v-else-if="result?.status === 'requested'" role="status" class="text-sm text-warning">This request is already reserved. Verify provider evidence before starting another test.</p>
        <p v-else-if="result" role="alert" class="text-sm text-error">{{ result.redactedError || result.errorClass || 'Provider rejected the test request.' }}</p>
      </div>
      <UButton
