  ...
  <div class="flex flex-col h-full min-h-0">
    <div v-if="conversation" class="p-4 border-b border-default">
      <div class="flex items-center gap-2">
        <span class="font-semibold truncate">{{ conversation.participant_name || 'Unknown' }}</span>
        <UBadge color="neutral" variant="subtle" size="xs">{{ conversation.platform }}</UBadge>
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-3">
      <div v-if="!conversation" class="text-sm text-muted">Select a conversation to view it.</div>
      <div
        v-for="m in messages" :key="m.id"
        class="flex" :class="m.direction === 'out' ? 'justify-end' : 'justify-start'"
      >
        <div
        ...
        >
          <div v-if="m.direction === 'in'" class="text-xs text-muted mb-0.5">{{ m.author_name || 'Unknown' }}</div>
          <p class="whitespace-pre-wrap break-words">{{ m.content }}</p>
          <div class="text-[10px] opacity-60 mt-1">{{ fmt(m.platform_timestamp || m.created_at) }}</div>
        </div>
      </div>
    </div>
  </div>
