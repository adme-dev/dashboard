...
const removeCollectionOpen = ref(false)
...
function removeCollection() {
  collections.value = collections.value.filter(item => item.id !== selectedCollection.value)
  removeCollectionOpen.value = false
}
...
<template>
  <section class="min-w-0 space-y-6">
        ...
        <div v-if="collection" class="space-y-5">
          <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <UButton
              ...
              @click="() => { removeCollectionOpen = true }"
            />
          </div>
          <div class="flex flex-wrap items-end gap-3">
            <UFormField v-if="records.length" label="Entry" class="min-w-0 basis-48 flex-1">
              <USelectMenu
          ...
          <fieldset v-if="record" :disabled="!canEdit || saving || pending" class="grid grid-cols-1 gap-4 @lg:grid-cols-2">
            ...
            <PageStudioContentAttributes v-model="record.attributes" :disabled="!canEdit || saving || pending" />
    ...
    <UModal v-model:open="removeCollectionOpen" title="Remove this collection?" description="All its entries will be removed from this draft when you save your changes.">
      <template #footer>
        <UButton
          label="Keep collection"
          color="neutral"
          variant="outline"
          @click="() => { removeCollectionOpen = false }"
        />
        <UButton
          label="Remove collection"
          color="error"
