<template>
  <div class="w-64 p-3">
    ...
    <div class="space-y-1 max-h-64 overflow-y-auto">
      <label
        class="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-gray-50 cursor-pointer text-sm"
      ...
      >
        ...
        />
        <span class="text-gray-700">None (default groups)</span>
      </label>
      ...
      <label
        ...
        :key="col.id"
        class="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-gray-50 cursor-pointer text-sm"
        :class="{ 'bg-blue-50': modelValue === col.id }"
      >
        <input
          type="radio"
        ...
        <span class="ml-auto text-xs text-gray-400">{{ columnTypeLabel(col) }}</span>
      </label>
    </div>

    <p v-if="!eligibleColumns.length" class="text-sm text-gray-500 text-center py-2">
