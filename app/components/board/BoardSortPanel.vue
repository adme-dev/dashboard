<template>
  <div class="w-72 p-3">
    ...
    <div v-if="modelValue.length" class="space-y-2 mb-3">
      ...
      >
        <span v-if="idx > 0" class="text-gray-400 text-xs">then</span>
        <span class="font-medium text-gray-700 truncate">{{ columnName(rule.columnId) }}</span>
        <button
          class="text-xs px-1.5 py-0.5 rounded border hover:bg-gray-100"
          :class="rule.direction === 'asc' ? 'text-blue-600 border-blue-200 bg-blue-50' : 'text-gray-600 border-gray-200'"
          @click="toggleDirection(rule.columnId)"
        >
          {{ rule.direction === 'asc' ? 'A-Z' : 'Z-A' }}
        </button>
        <button class="ml-auto text-gray-400 hover:text-gray-600 shrink-0" @click="removeSort(rule.columnId)">
    ...
    <div v-if="availableColumns.length" class="border-t pt-3">
      <div class="flex items-center gap-2">
        <select v-model="newColumnId" class="flex-1 text-sm border rounded-md px-2 py-1.5 bg-white">
    ...
    <p v-else-if="!modelValue.length" class="text-sm text-gray-500 text-center py-2">
      No sortable columns
    </p>
  </div>
</template>
