...
async function saveNotes(text: string) {
  ...
  try {
    ...
    savedTimer = setTimeout(() => { notesState.value = 'idle' }, 2000)
  } catch (e: any) {
    notesState.value = 'idle'
    toast.add({ title: 'Failed to save notes', description: e?.message, color: 'error' })
  }
}
        ...
        <header class="px-6 py-4 border-b border-default">
          <div class="flex items-center justify-between gap-3">
            <div class="flex items-center gap-2 min-w-0">
              ...
              <h2 class="text-base font-semibold truncate">{{ lead.form_name || lead.form_id || 'Lead' }}</h2>
            </div>
        ...
        <div class="flex-1 overflow-auto p-6 space-y-6">
          <section>
            ...
            <dl v-if="fieldRows.length" class="grid grid-cols-3 gap-x-3 gap-y-2 text-sm">
              <template v-for="[k, v] in fieldRows" :key="k">
                <dt class="text-muted col-span-1">{{ prettyKey(k) }}</dt>
                <dd class="break-all col-span-2">{{ prettyValue(v) }}</dd>
              </template>
            </dl>
            <p v-else class="text-muted text-sm">No field data.</p>
          </section>
          ...
          <section>
            <div class="flex items-center justify-between mb-2">
              <h3 class="text-xs font-semibold uppercase text-muted">Notes</h3>
