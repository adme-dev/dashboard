  ...
  <section class="@container overflow-hidden rounded-xl border border-default bg-default">
    <div class="border-b border-default px-4 py-4">
      <div class="grid grid-cols-1 gap-4 @lg:grid-cols-2 @3xl:grid-cols-4 @5xl:grid-cols-7">
        ...
        <UFormField label="Campaign state">
          <USelect
            :model-value="filters.campaignStatus"
            :items="campaignStatusOptions"
            class="w-full"
            @update:model-value="emit('updateFilter', 'campaignStatus', String($event))"
          />
        </UFormField>
    ...
    <div v-if="!loading && items.length === 0" class="px-6 py-14 text-center" role="status">
      ...
      <h3 class="mt-3 text-sm font-semibold">
        No campaigns match this review
      </h3>
    ...
    >
      <template #client-cell="{ row }">
        <div class="min-w-36">
          <p class="text-sm font-medium">
            {{ row.original.client?.name || 'Unmapped client' }}
          </p>
          <p class="text-xs text-muted">
            {{ row.original.accountName || row.original.customerId }}
          </p>
        </div>
      </template>
      <template #campaignName-cell="{ row }">
        <UButton
          ...
          @click="emit('openCampaign', row.original.id)"
        >
      ...
      <template #risks-cell="{ row }">
        <div v-if="row.original.risks.length" class="flex max-w-52 flex-wrap gap-1">
          <UBadge
            v-for="risk in row.original.risks.slice(0, 2)"
            :key="risk"
            color="warning"
            variant="subtle"
            size="xs"
          ...
          <UBadge
            v-if="row.original.risks.length > 2"
            color="neutral"
            variant="subtle"
            size="xs"
          >
