...
const stages = ['draft', 'submitted', 'in_progress', 'review', 'complete'] as const
    ...
    <div v-else>
      ...
      <div v-if="recentBriefs.length" class="border-t border-[var(--ui-border)] pt-3 space-y-2">
        <div v-for="brief in recentBriefs" :key="brief.id" class="flex items-center gap-2 text-sm">
          <span class="w-1.5 h-1.5 rounded-full shrink-0" :class="stageColors[brief.status]?.split(' ')[0] || 'bg-neutral-300'" />
