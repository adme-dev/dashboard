...
// Customer-health (churn-risk) breakdown for a person/company. Agency-only:
// health endpoints live under /api/crm — self-guard so this renders nothing in
// the client portal. Reuses the shared crm_scores grade column (Hot/Warm/Cold),
// relabelled for health context.
...
const gradeLabel: Record<string, string> = { Hot: 'Healthy', Warm: 'At risk', Cold: 'Critical' }
const gradeColor: Record<string, string> = { Hot: 'success', Warm: 'warning', Cold: 'error' }
const components = computed(() => score.value ? [
  { label: 'Engagement', value: score.value.engagement_score, max: 35 },
  { label: 'Support', value: score.value.intent_score, max: 20 },
  { label: 'Relationship', value: score.value.fit_score, max: 20 },
  { label: 'Contract', value: score.value.recency_score, max: 25 },
] : [])
...
async function recompute() {
  ...
  try {
    ...
    toast.add({ title: 'Could not recompute health', description: (e as Error)?.message, color: 'error' })
  } finally {
  ...
  <div v-if="isAgency">
    <div class="flex items-center justify-between gap-2 mb-3">
      <div class="flex items-center gap-2">
        <h3 class="text-sm font-semibold text-highlighted">Customer health</h3>
        <UBadge v-if="score" :color="(gradeColor[score.grade] as any)" variant="subtle" size="sm">
          {{ gradeLabel[score.grade] }} · {{ score.total_score }}
        </UBadge>
      </div>
      <UButton
    ...
    <div v-if="!score" class="rounded-lg border border-dashed border-default py-4 text-center text-sm text-muted">
      No health score yet — auto-computed for customers, or click Recompute.
    </div>
