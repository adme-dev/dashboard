...
function findSection(rows, matcher) {
  const queue = [...rows];
  while (queue.length) {
...
function sumValues(values, upToIndex) {
...
function createBreakdown(section, lastIndex, previousIndex, ytdIndex, options = {}) {
  const items = collectBreakdownItems(section, options);
  if (items.length === 0) return [];
  const normalize = Boolean(options.normalizeSign);
  const data = items.map((item) => {
    var _a, _b;
    const currentRaw = (_a = item.values[lastIndex]) != null ? _a : 0;
    const priorRaw = previousIndex >= 0 ? (_b = item.values[previousIndex]) != null ? _b : 0 : 0;
...
function buildInsights({
  ...
  if (topExpense) {
    ...
    const expenseDelta2 = topExpense.month - topExpense.previousMonth;
    if (Math.abs(expenseDelta2) > Math.abs(topExpense.previousMonth) * 0.1) {
      const direction = expenseDelta2 >= 0 ? "rose" : "fell";
      insights.push(`${topExpense.name} ${direction} by ${formatCurrency(Math.abs(expenseDelta2))} month-over-month.`);
    }
  }
...
const pnlDetailed_get = eventHandler(async (event) => {
  ...
  const requestedToDate = parseDateInput(typeof query.toDate === "string" ? query.toDate : void 0);
  const basis = typeof query.basis === "string" && query.basis.toLowerCase() === "cash" ? "Cash" : "Accrual";
  const monthEnd = requestedToDate != null ? requestedToDate : getDefaultMonthEnd();
  ...
  try {
    ...
    const statusCode = ((_a = err == null ? void 0 : err.response) == null ? void 0 : _a.status) || 500;
    const detail = ((_b = err == null ? void 0 : err.response) == null ? void 0 : _b.data) || (err == null ? void 0 : err.message);
  ...
  const reportTable = (_e = (_c = report == null ? void 0 : report.reports) == null ? void 0 : _c[0]) != null ? _e : (_d = report == null ? void 0 : report.Reports) == null ? void 0 : _d[0];
  const tableRows = reportTable ? (_g = (_f = reportTable.rows) != null ? _f : reportTable.Rows) != null ? _g : [] : [];
  const parsedRows = tableRows.filter((row) => getRowType(row).toLowerCase() !== "header").map(parseRow);
  ...
  const revenueMonth = (_h = revenueRow == null ? void 0 : revenueRow.values[lastIndex]) != null ? _h : 0;
  const revenuePrev = previousIndex >= 0 ? (_i = revenueRow == null ? void 0 : revenueRow.values[previousIndex]) != null ? _i : 0 : 0;
  const revenueYtd = revenueRow ? sumValues(revenueRow.values, ytdIndex) : revenueMonth;
  const cogsMonth = (_j = cogsRow == null ? void 0 : cogsRow.values[lastIndex]) != null ? _j : 0;
  const cogsPrev = previousIndex >= 0 ? (_k = cogsRow == null ? void 0 : cogsRow.values[previousIndex]) != null ? _k : 0 : 0;
  const cogsYtd = cogsRow ? sumValues(cogsRow.values, ytdIndex) : cogsMonth;
  const grossMonth = (_l = grossProfitRow == null ? void 0 : grossProfitRow.values[lastIndex]) != null ? _l : revenueMonth + cogsMonth;
  const grossPrev = previousIndex >= 0 ? (_m = grossProfitRow == null ? void 0 : grossProfitRow.values[previousIndex]) != null ? _m : revenuePrev + cogsPrev : grossMonth;
  const grossYtd = grossProfitRow ? sumValues(grossProfitRow.values, ytdIndex) : revenueYtd + cogsYtd;
  const operatingExpensesMonthRaw = (_n = expenseRow == null ? void 0 : expenseRow.values[lastIndex]) != null ? _n : 0;
  const operatingExpensesPrevRaw = previousIndex >= 0 ? (_o = expenseRow == null ? void 0 : expenseRow.values[previousIndex]) != null ? _o : 0 : 0;
  ...
  const netProfitMonth = (_p = netProfitRow == null ? void 0 : netProfitRow.values[lastIndex]) != null ? _p : grossMonth - operatingExpensesMonthRaw;
  const netProfitPrev = previousIndex >= 0 ? (_q = netProfitRow == null ? void 0 : netProfitRow.values[previousIndex]) != null ? _q : grossPrev - operatingExpensesPrevRaw : netProfitMonth;
  const netProfitYtd = netProfitRow ? sumValues(netProfitRow.values, ytdIndex) : grossYtd - operatingExpensesYtdRaw;
  const totalExpenseSeries = (_r = expenseRow == null ? void 0 : expenseRow.values.map((value) => Math.abs(value))) != null ? _r : new Array(periodLabels.length).fill(0);
  const revenueSeries = (_s = revenueRow == null ? void 0 : revenueRow.values) != null ? _s : new Array(periodLabels.length).fill(0);
  const netProfitSeries = (_t = netProfitRow == null ? void 0 : netProfitRow.values) != null ? _t : new Array(periodLabels.length).fill(0);
  const breakdownRevenue = createBreakdown(revenueSection, lastIndex, previousIndex, ytdIndex, {
    normalizeSign: false,
    fallbackRows: flattened,
    filter: (row) => {
      var _a2;
      const value = (_a2 = row.values[lastIndex]) != null ? _a2 : 0;
      return value > 0;
    }
  });
  ...
  const periodDetails = periodLabels.map((label, index) => {
    ...
    const revenue = (_a2 = revenueRow == null ? void 0 : revenueRow.values[index]) != null ? _a2 : 0;
    const directCosts = Math.abs((_b2 = cogsRow == null ? void 0 : cogsRow.values[index]) != null ? _b2 : 0);
    const operatingExpenses = Math.abs((_c2 = expenseRow == null ? void 0 : expenseRow.values[index]) != null ? _c2 : 0);
    const grossProfit = revenue - directCosts;
    const netProfit = (_e2 = (_d2 = netProfitRow == null ? void 0 : netProfitRow.values) == null ? void 0 : _d2[index]) != null ? _e2 : grossProfit - operatingExpenses;
    return {
      label,
      revenue,
      directCosts,
...
export { pnlDetailed_get as default };
//# sourceMappingURL=pnl-detailed.get.mjs.map
