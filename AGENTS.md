...
- **Lead Capture & Routing**: real-time inbound lead inbox at `/agency/leads`. Native Google Ads webhooks (per-client URL+key), Meta verify-token endpoint (live; full ingestion gated by Meta App Review for `leads_retrieval`), dedicated client-scoped inbound email addresses, generic webhook for Zapier/Make/n8n/custom (`/api/leads/webhook/generic/<token>`), CSV importer for Meta Lead Center exports, and manual entry. Form rules fan out to Slack / email / outbound webhook / Google Sheets / client portal / auto-assign user. Per-destination filters (`field_data.budget gt 5000`), delays up to 24h, idempotency by `source_lead_id`, `is_test` flagging hides Google "Send test data" from default inbox view. Form picker dropdown via OAuth (Google fully working today; Meta gated). OAuth-based `pages_show_list,pages_manage_ads,leads_retrieval,business_management` scope set requested — activation requires the operator to reconnect each Meta account post-approval. See `server/api/leads/`, `server/utils/leads/`, `app/components/leads/`. Lead migrations include 087, 090, 091 and universal email migrations 315–324.
...
**Whenever you build or edit a form, first invoke the `frontend-design` skill** at `~/.Codex/plugins/marketplaces/Codex-plugins-official/plugins/frontend-design/skills/frontend-design/SKILL.md` and apply its design principles (typography, hierarchy, spacing, avoiding generic AI aesthetics) before writing or modifying any form fields. This is non-negotiable for any form-touching work in this project.
...
- **Field grids** — start with `grid grid-cols-1 gap-4`; add a second column only when the form's available width can support both controls comfortably. On full-width pages, use a responsive viewport variant such as `sm:grid-cols-2`. Inside `USlideover`, `UModal`, sidebars, cards, or other constrained surfaces, mark the form or section with `@container` and use a container variant such as `@lg:grid-cols-2` instead of viewport breakpoints. Never use an unconditional `grid-cols-2` in a narrow surface.
- **Control width** — `UInput`, `USelect`, `USelectMenu`, `UTextarea`, and date-picker triggers use `class="w-full"` inside form grids unless the control is intentionally compact. Long-text and unpaired fields span the expanded grid with the matching container or viewport variant (for example `@lg:col-span-2`).
...
## Universal email lead ingestion

Email ingestion is a transport into the existing canonical lead pipeline, not
a second CRM system.

Architecture:

- When activated, Cloudflare Email Routing sends the `leads.xeroflow.io`
  catch-all to `workers/email-lead-intake/`.
- Shared MIME, provider adapter, canonical contract, signing, encryption, and
  telemetry code lives under `shared/leads/email/`.
- Signed Worker boundaries are `server/api/internal/leads/email-policy.post.ts`,
  `email-stage.post.ts`, `email-stage-confirm.post.ts`,
  `email-ingest.post.ts`, and `email-telemetry.post.ts`.
- Endpoint/replay APIs live under `server/api/leads/email-endpoints/` and
  `server/api/leads/email-ingestions/`.
- Canonical ingestion, recovery, and health are implemented in
  `server/utils/leads/emailIngestion.ts`,
  `server/utils/leads/emailRecovery.ts`, and
  `server/utils/leads/emailHealth.ts`.
- Schema is additive across migrations 315–324. Operator procedure is
  `docs/runbooks/email-lead-ingestion.md`.

Safety and privacy rules:

- Resolve endpoint/client/form only from the server-side recipient token. Never
  trust message content, headers, provider hints, or AI output for tenancy.
- Hash provider external identity and Message-ID before the Worker sends the
  envelope.
- Deterministic parsing is default. AI is an optional structured extraction
  fallback requiring platform capability and privacy approval; it never replies
  to customers or drafts messages.
- Stage raw MIME only after a durable reservation, encrypt it before private R2
  storage, and delete it after accepted/duplicate completion or seven-day
  expiry.
- Logs, alerts, audits, health, and portal projections must remain content-free.
- Worker ingress, recovery, and manual replay must reuse the original
  endpoint-scoped idempotency identity.
- New provider adapters must be bounded/deterministic, make no remote fetches,
  add sanitised fixtures, and pass
...
- Future authenticated transports must produce the shared canonical envelope
  and call the existing canonical ingestion boundary. Never insert directly
  into leads or create another rule/CRM/delivery pipeline.
- Pages compatibility date `2024-12-01` does not imply bindings populate
  `process.env`. Read deployed bindings from `event.context.cloudflare.env`,
  retaining `process.env` only as a local/test fallback.
