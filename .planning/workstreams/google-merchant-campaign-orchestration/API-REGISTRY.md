...
|---|---|---|---|---|
| Merchant API | `merchantapi.googleapis.com` | Required for Merchant account, data-source, product and issue operations | Console enablement URL supplied; enablement and registration not proven | Verify project ownership, enable, then register approved agency Merchant topology |
| Google Ads API | `googleads.googleapis.com` | Required for campaign/PMax reads and writes | Application calls Ads REST v23 and has a configured developer-token path | Verify enabled in the same OAuth project and record token access level/owner securely |

## Conditional services

| API | Service name | When needed | Decision |
|---|---|---|---|
| Data Manager API | `datamanager.googleapis.com` | First-party conversion or audience ingestion included in campaign readiness/monitoring | Existing XeroFlow capability; verify current project and access, do not duplicate |
...
- [ ] `adwords` and `content` scopes present in the relevant connection flow.
