...
const directory = await mkdtemp(join(tmpdir(), 'cms-preparation-browser-'))
let server
try {
  ...
  const files = ['main.js', 'App.vue', 'style.css', 'index.html', 'vite.config.mjs', 'setup.mjs']
  const templates = new Map()
  for (const file of files) {
    const source = await readFile(join(root, 'test/fixtures/cmsPreparationBrowser', `${file}.txt`), 'utf8')
  ...
  for (const file of ['setup.mjs']) {
    ...
    await new Promise((resolve, reject) => {
      ...
      child.once('exit', (code) => {
        ...
        if (code === 0) resolve()
        else reject(new Error(`CMS preparation browser scenario failed: ${file} (${code})`))
      })
    })
  }
  console.log('CMS preparation: all real-browser scenarios passed.')
} finally {
  await server?.close()
